...
void slave_biharmonic_wk_scalar_(param_t *param_s) {
  ...
  for (ie = 0; ie < nete - nets + 1; ie++) {
    ...
    for (q = 0; q <= q_n; q++) {
      ...
      src_buf = gl_buf + is + q_beg*NLEV*NP + (k_beg + q*(q*NLEV - k_beg))*NP;
      ...
      src_buf = gl_buf + iee + q_beg*NLEV*NP + (k_beg + q*(q*NLEV - k_beg))*NP;
      pe_put(src_buf, buf, k_n*NP*sizeof(double));
      dma_syn();
      // ------------------------------ North --------------------------------//
      for (k = 0; k < k_n; k++) {
      ...
      src_buf = gl_buf + in + q_beg*NLEV*NP + (k_beg + q*(q*NLEV - k_beg))*NP;
      pe_put(src_buf, buf, k_n*NP*sizeof(double));
      dma_syn();
      // ------------------------------ West ---------------------------------//
      for (k = 0; k < k_n; k++) {
      ...
      src_buf = gl_buf + iw + q_beg*NLEV*NP + (k_beg + q*(q*NLEV - k_beg))*NP;
      pe_put(src_buf, buf, k_n*NP*sizeof(double));
      dma_syn();
      // ---------------------------- SWEST -------------------------------- //
      ...
      for (ll = swest + 3*max_corner_elem - 1; ll < (swest + 4*max_corner_elem - 1); ll++) {
        if (putmap[ll] != -1) {
          for (k = 0; k < k_n; k++) {
            int pos_qtens = k*NP*NP + (NP - 1)*NP + (NP - 1);
            buf_1[k] = qtens[pos_qtens];
          }
          src_buf = gl_buf + putmap[ll] + q_beg*NLEV+ (k_beg + q*(q*NLEV - k_beg));
          pe_put(src_buf, buf_1, k_n*sizeof(double));
