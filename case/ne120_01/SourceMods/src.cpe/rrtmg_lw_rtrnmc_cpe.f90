...
!      use parkind, only : jpim, jprb 
      use parrrtm, only : mg, mod_nbndlw => nbndlw, mod_ngptlw => ngptlw
      use rrlw_con, only: mod_fluxfac => fluxfac, mod_heatfac => heatfac
      use rrlw_wvn, only: mod_delwave => delwave, mod_ngs => ngs
      use rrlw_tbl, only: mod_tblint => tblint, mod_bpade => bpade, tau_tbl, exp_tbl, tfn_tbl
...
!-----------------------------------------------------------------------------
      subroutine rtrnmc(nlayers, istart, iend, iout, pz, semiss, ncbands, &
                        pm_cldfmc, pm_taucmc, planklay, planklev, plankbnd, &
                        pwvcm, pm_fracs, iaer, taug, taua, &
...
! Atmosphere
      ...
      real(kind=r8), intent(in) :: plankbnd(:)          ! 
                                                        !    Dimensions: (nbndlw)
      real(kind=r8), intent(in) :: pm_fracs(:,:)           ! 
                                                        !    Dimensions: (nlayers,ngptw)
      ! real(kind=r8), intent(in) :: pm_taut(:,:)            ! gaseous + aerosol optical depths
      !                                                   !    Dimensions: (nlayers,ngptlw)

! Clouds
      integer, intent(in) :: ncbands                    ! number of cloud spectral bands
      real(kind=r8), intent(in) :: pm_cldfmc(:,:)          ! layer cloud fraction [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
      real(kind=r8), intent(in) :: pm_taucmc(:,:)          ! layer cloud optical depth [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
...
! ----- I/O Extension -----
      ...
      real(kind=r8), intent(in) :: taua(:,:)
                                                        !    Dimensions: (nlayers,nbndlw)
      real(kind=r8), intent(in) :: taug(:,:)
                                                        !    Dimensions: (nlayers,ngptlw)
      integer, intent(in) :: icldlyr(:)                 ! flag for cloud in layer
...
! Declarations for radiative transfer
      ...
      real(kind=r8) :: odcld(nlayers)


      real(kind=r8) :: secdiff(mod_nbndlw)                   ! secant of diffusivity angle
      real(kind=r8) :: a0(mod_nbndlw),a1(mod_nbndlw),a2(mod_nbndlw)  ! diffusivity angle adjustment coefficients
...
! ----- LDM -----
      real(kind=r8) :: cldfmc(nlayers)
      real(kind=r8) :: taucmc(nlayers)
      real(kind=r8) :: taut(nlayers)
      real(kind=r8) :: fracs(nlayers)
      integer :: nbndlw, ngptlw
      real(kind=r8) :: fluxfac, heatfac
      real(kind=r8) :: tblint, bpade
      real(kind=r8) :: delwave(mod_nbndlw)
      integer :: ngs(mod_nbndlw)
...
! angle.  This initial value is redefined below for some bands.
      ...
      tblint = mod_tblint
      bpade = mod_bpade
      call dmemcpy(delwave, mod_delwave, nbndlw * kind(delwave))
      call dmemcpy(ngs, mod_ngs, nbndlw * kind(ngs))
...
! over a wide range of precipitable water values.
      a0(1) = 1.66_r8;  a0(2) = 1.55_r8;   a0(3) = 1.58_r8;   a0(4) = 1.66_r8
      a0(5) = 1.54_r8;  a0(6) = 1.454_r8;  a0(7) = 1.89_r8;   a0(8) = 1.33_r8
      a0(9) = 1.668_r8; a0(10) = 1.66_r8;  a0(11) = 1.66_r8;  a0(12) = 1.66_r8
      a0(13) = 1.66_r8; a0(14) = 1.66_r8;  a0(15) = 1.66_r8;  a0(16) = 1.66_r8

      a1(1) = 0.00_r8;    a1(2) = 0.25_r8;  a1(3) = 0.22_r8;   a1(4) = 0.00_r8
      a1(5) = 0.13_r8;    a1(6) = 0.446_r8; a1(7) = -0.10_r8;  a1(8) = 0.40_r8
      a1(9) = -0.006_r8;  a1(10) = 0.00_r8; a1(11) = 0.00_r8;  a1(12) = 0.00_r8
      a1(13) = 0.00_r8;   a1(14) = 0.00_r8; a1(15) = 0.00_r8;  a1(16) = 0.00_r8

      a2(1) =  0.00_r8;  a2(2) = -12.0_r8;   a2(3) = -11.7_r8; a2(4) = 0.00_r8
      a2(5) = -0.72_r8;  a2(6) = -0.243_r8;  a2(7) = 0.19_r8;  a2(8) = -0.062_r8
      a2(9) = 0.414_r8;  a2(10) = 0.00_r8;   a2(11) = 0.00_r8; a2(12) = 0.00_r8
      a2(13) = 0.00_r8;  a2(14) = 0.00_r8;   a2(15) = 0.00_r8; a2(16) = 0.00_r8
...
! Radiative transfer starts here.
         ...
         if (iaer .eq. 0) then
            call dmemcpy(taut, taug(:,igc), nlayers * kind(taut))
         else if (iaer .eq. 10) then
            call dmemcpy(taut, taug(:,igc), nlayers * kind(taut))
            do lev = 1, nlayers
                taut(lev) = taut(lev) + taua(lev,iband)
            enddo
         endif
         call dmemcpy(fracs, pm_fracs(:,igc), nlayers * kind(fracs))
         do lev = 1, nlayers
            cldfmc(lev) = pm_cldfmc(igc, lev)
            taucmc(lev) = pm_taucmc(igc, lev)

            if (cldfmc(lev) .eq. 1._r8) then
               ib = iband
               odcld(lev) = secdiff(ib) * taucmc(lev)
               transcld = exp(-odcld(lev))
               abscld = 1._r8 - transcld
               efclfrac(lev) = abscld * cldfmc(lev)
            else
               odcld(lev) = 0.0_r8
               efclfrac(lev) = 0.0_r8
            endif
...
! Set heating rate to zero in top layer
      ...
      end module rrtmg_lw_rtrnmc_cpe
