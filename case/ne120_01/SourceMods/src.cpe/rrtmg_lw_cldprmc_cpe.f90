...
! ------------------------------------------------------------------------------
      subroutine cldprmc(nlayers, inflag, iceflag, liqflag, pm_cldfmc, &
                         pm_ciwpmc, pm_clwpmc, reicmc, dgesmc, relqmc, ncbands, pm_taucmc)
! ------------------------------------------------------------------------------
...
! ------- Input -------
      ...
      real(kind=r8), intent(in) :: pm_cldfmc(:,:)       ! cloud fraction [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
      real(kind=r8), intent(in) :: pm_ciwpmc(:,:)       ! cloud ice water path [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
      real(kind=r8), intent(in) :: pm_clwpmc(:,:)       ! cloud liquid water path [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
...
! ------- Output -------
      ...
      real(kind=r8), intent(inout) :: pm_taucmc(:,:)    ! cloud optical depth [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
...
! ------- Local -------
      ...
      real(kind=r8) :: abscoice(mod_ngptlw)     ! ice absorption coefficients
      real(kind=r8) :: abscoliq(mod_ngptlw)     ! liquid absorption coefficients
      ...
      real(kind=r8) :: eps                      ! epsilon
      real(kind=r8) :: cldmin                   ! minimum value for cloud quantities

! ------- LDM -------
      integer :: ngptlw
      integer :: ngb(mod_ngptlw)
      real(kind=r8) :: cldfmc(mod_ngptlw)
      real(kind=r8) :: ciwpmc(mod_ngptlw)
      real(kind=r8) :: clwpmc(mod_ngptlw)
      real(kind=r8) :: taucmc(mod_ngptlw)
...
!                     coefficients for the input effective radius.
      ...
      cldmin = 1.e-80_r8
      ngptlw = mod_ngptlw
      call dmemcpy(ngb, mod_ngb, ngptlw * kind(ngb))
...
! Main layer loop
      do lay = 1, nlayers
        call dmemcpy(cldfmc, pm_cldfmc(:,lay), ngptlw * kind(cldfmc))
        call dmemcpy(ciwpmc, pm_ciwpmc(:,lay), ngptlw * kind(ciwpmc))
        call dmemcpy(clwpmc, pm_clwpmc(:,lay), ngptlw * kind(clwpmc))
        call dmemcpy(taucmc, pm_taucmc(:,lay), ngptlw * kind(taucmc))

        do ig = 1, ngptlw
          cwp = ciwpmc(ig) + clwpmc(ig)
          if (cldfmc(ig) .ge. cldmin .and. &
             (cwp .ge. cldmin .or. taucmc(ig) .ge. cldmin)) then
...
! Calculation of absorption coefficients due to water clouds.
               ...
               taucmc(ig) = ciwpmc(ig) * abscoice(ig) + &
                                clwpmc(ig) * abscoliq(ig)

            endif
         endif
         enddo
         call dmemcpy(pm_taucmc(:,lay), taucmc, ngptlw * kind(taucmc))
      enddo

      end subroutine cldprmc

      end module rrtmg_lw_cldprmc_cpe
