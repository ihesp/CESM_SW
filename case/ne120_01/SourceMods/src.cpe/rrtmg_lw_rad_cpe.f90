...
!      use parkind, only : jpim, jprb 
      ...
      use rrtmg_lw_cldprmc_cpe, only: cldprmc
! Move call to rrtmg_lw_ini and following use association to 
! GCM initialization area
!      use rrtmg_lw_init, only: rrtmg_lw_ini
      use rrtmg_lw_rtrnmc_cpe, only: rtrnmc
      use rrtmg_lw_setcoef_cpe, only: setcoef
      use rrtmg_lw_taumol_cpe, only: taumol
      use rrtmg_lw_rad_mpe, only: rrtmg_lw_param
...
! --------- Modules ----------
      ...
      use parrrtm, only : nbndlw, mod_ngptlw => ngptlw, maxxsec, mxmol, nmol
...
! Atmosphere
      ...
      real(kind=r8) :: wkl(nmol,nlay)           ! molecular amounts (mol/cm-2)
...
! Atmosphere - setcoef
      ...
      pointer(colh2o_ptr, colh2o)
      pointer(colco2_ptr, colco2)
      pointer(colo3_ptr, colo3)
      pointer(coln2o_ptr, coln2o)
      pointer(colco_ptr, colco)
      pointer(colch4_ptr, colch4)
      pointer(colo2_ptr, colo2)
      pointer(colbrd_ptr, colbrd)
      pointer(selffac_ptr, selffac)
      pointer(selffrac_ptr, selffrac)
      pointer(forfac_ptr, forfac)
      pointer(forfrac_ptr, forfrac)
      pointer(minorfrac_ptr, minorfrac)
      pointer(scaleminor_ptr, scaleminor)
      pointer(scaleminorn2_ptr, scaleminorn2)
      ...
      pointer(fac00_ptr, fac00)
      pointer(fac01_ptr, fac01)
      pointer(fac10_ptr, fac10)
      pointer(fac11_ptr, fac11)
      pointer(rat_h2oco2_ptr, rat_h2oco2)
      pointer(rat_h2oco2_1_ptr, rat_h2oco2_1)
      pointer(rat_h2oo3_ptr, rat_h2oo3)
      pointer(rat_h2oo3_1_ptr, rat_h2oo3_1)
      pointer(rat_h2on2o_ptr, rat_h2on2o)
      pointer(rat_h2on2o_1_ptr, rat_h2on2o_1)
      pointer(rat_h2och4_ptr, rat_h2och4)
      pointer(rat_h2och4_1_ptr, rat_h2och4_1)
      pointer(rat_n2oco2_ptr, rat_n2oco2)
      pointer(rat_n2oco2_1_ptr, rat_n2oco2_1)
      pointer(rat_o3co2_ptr, rat_o3co2)
      pointer(rat_o3co2_1_ptr, rat_o3co2_1)
...
! LDM
      integer :: myid
      integer :: ngptlw
      integer :: icldlyr(nlay)

      call  athread_get_vid(myid)

      colh2o_ptr = loc(totufluxs)
      colco2_ptr = loc(colh2o) + nlay * 8
      colo3_ptr = loc(colco2) + nlay * 8
      coln2o_ptr = loc(colo3) + nlay * 8
      colco_ptr = loc(coln2o) + nlay * 8
      colch4_ptr = loc(colco) + nlay * 8
      colo2_ptr = loc(colch4) + nlay * 8
      colbrd_ptr = loc(colo2) + nlay * 8
      selffac_ptr = loc(colbrd) + nlay * 8
      selffrac_ptr = loc(selffac) + nlay * 8
      forfac_ptr = loc(selffrac) + nlay * 8
      forfrac_ptr = loc(forfac) + nlay * 8
      minorfrac_ptr = loc(forfrac) + nlay * 8
      scaleminor_ptr = loc(minorfrac) + nlay * 8
      scaleminorn2_ptr = loc(scaleminor) + nlay * 8

      fac00_ptr = loc(totdfluxs)
      fac01_ptr = loc(fac00) + nlay * 8
      fac10_ptr = loc(fac01) + nlay * 8
      fac11_ptr = loc(fac10) + nlay * 8
      rat_h2oco2_ptr = loc(fac11) + nlay * 8
      rat_h2oco2_1_ptr = loc(rat_h2oco2) + nlay * 8
      rat_h2oo3_ptr = loc(rat_h2oco2_1) + nlay * 8
      rat_h2oo3_1_ptr = loc(rat_h2oo3) + nlay * 8
      rat_h2on2o_ptr = loc(rat_h2oo3_1) + nlay * 8
      rat_h2on2o_1_ptr = loc(rat_h2on2o) + nlay * 8
      rat_h2och4_ptr = loc(rat_h2on2o_1) + nlay * 8
      rat_h2och4_1_ptr = loc(rat_h2och4) + nlay * 8
      rat_n2oco2_ptr = loc(rat_h2och4_1) + nlay * 8
      rat_n2oco2_1_ptr = loc(rat_n2oco2) + nlay * 8
      rat_o3co2_ptr = loc(rat_n2oco2_1) + nlay * 8
      rat_o3co2_1_ptr = loc(rat_o3co2) + nlay * 8

      ngptlw = mod_ngptlw

      if (myid + 1 <= ncol) then
          allocate(fracs(nlay,ngptlw), taug(nlay,ngptlw))
          allocate(cldfmc(ngptlw,nlay), ciwpmc(ngptlw,nlay), clwpmc(ngptlw,nlay), taucmc(ngptlw,nlay))
      endif
...
!  Vertical indexing goes from bottom to top
         ...
         do k = 0, nlay
            ...
            call dmemcpy(uflxs(:,iplon,k+1), totufluxs(:, k), nbndlw * kind(uflxs))
            call dmemcpy(dflxs(:,iplon,k+1), totdfluxs(:, k), nbndlw * kind(dflxs))
         enddo
         do k = 0, nlay-1
            hr(iplon,k+1) = htr(k)
...
! LDM
      real(kind=r8) :: loc_cldfmc(ngptlw)
...
! convert effective radius to generalized effective size using method of Mitchell, JAS, 2002:
         ...
         icldlyr(:) = 0
         do l = 1, nlay-1
            call dmemcpy(loc_cldfmc(:), cldfmcl(:,iplon,nlay-l), ngptlw * kind(cldfmc))
            call dmemcpy(cldfmc(:,l), loc_cldfmc(:), ngptlw * kind(cldfmc))
            call dmemcpy(taucmc(:,l), taucmcl(:,iplon,nlay-l), ngptlw * kind(taucmc))
            call dmemcpy(ciwpmc(:,l), ciwpmcl(:,iplon,nlay-l), ngptlw * kind(ciwpmc))
            call dmemcpy(clwpmc(:,l), clwpmcl(:,iplon,nlay-l), ngptlw * kind(clwpmc))

            do ig = 1, ngptlw
               if (loc_cldfmc(ig) .eq. 1._r8) then
                  icldlyr(l) = 1
               endif
            enddo
...
! If an extra layer is being used in RRTMG, set all cloud properties to zero in the extra layer.
         ...
         call dmemset(cldfmc(:,nlay), 0, ngptlw * kind(cldfmc))
         call dmemset(taucmc(:,nlay), 0, ngptlw * kind(taucmc))
         call dmemset(ciwpmc(:,nlay), 0, ngptlw * kind(ciwpmc))
         call dmemset(clwpmc(:,nlay), 0, ngptlw * kind(clwpmc))
         reicmc(nlay) = 0.0_r8
         dgesmc(nlay) = 0.0_r8
         relqmc(nlay) = 0.0_r8
         taua(nlay,:) = 0.0_r8
      ...
      subroutine rrtmg_lw_1param (gl_param)
      type(rrtmg_lw_param), intent(inout) :: gl_param
      ...
      call dmemcpy(param, gl_param, loc(param%end_type) - loc(param))

      ! call set_mpi_rank(param%mpi_rank)
      ! call set_func_ptr
      call rrtmg_lw &
            (param%lchnk   ,param%ncol    ,param%nlay    ,param%icld    ,                   &
             param%play    ,param%plev    ,param%tlay    ,param%tlev    ,param%tsfc    ,param%h2ovmr  , &
             param%o3vmr   ,param%co2vmr  ,param%ch4vmr  ,param%o2vmr   ,param%n2ovmr  ,&
             param%cfc11vmr,param%cfc12vmr, &
             param%cfc22vmr,param%ccl4vmr ,param%emis    ,param%inflglw ,param%iceflglw,param%liqflglw, &
             param%cldfmcl ,param%taucmcl ,param%ciwpmcl ,param%clwpmcl ,param%reicmcl ,param%relqmcl , &
             param%tauaer  , &
             param%uflx    ,param%dflx    ,param%hr      ,param%uflxc   ,param%dflxc,  param%hrc, param%uflxs, param%dflxs )

      call reset_exp_data

      end subroutine rrtmg_lw_1param

      end module rrtmg_lw_rad_cpe
