...
! added by zengyh 2019-04-07:
! such as KYU,KXU,tavg variables.
   ! use advection
   use depend_mod
...
!-----------------------------------------------------------------------
  !yangtu12 29 (8) (r8)
   real(8)::start,finish
   real(8)::start_call,finish_call
   integer::jbegin,jend,ibegin,iend,array_x,array_y,array_z
...
!public set by zengyh 2019-04-09: 
   ...
   real (r8),public ::         &
      ah, ABS_ADD1,ABS_ADD2,              &! horizontal tracer   mixing coefficient
...
!-----------------------------------------------------------------------
 ...
 !  call getlooptime(start)

   call broadcast_scalar(lvariable_hmixu, master_task)
   call broadcast_scalar(am,              master_task)


 ! call getlooptime(finish) 
 ! if (my_task == master_task) then 
 !      WRITE (*,*) 'In hmix_del4, loop3  costs : ',finish-start,' (us);'
 ! endif
...
!***********************************************************************
!BOP
! !IROUTINE: advuhdiffu_del4
! !INTERFACE:

 subroutine advuhdiffu_del4(k, LUK, LVK, HDUK, HDVK, WUK, UUU, VVV, UMIXK, VMIXK, this_block)

! !DESCRIPTION:
! Combine advu and hdiffu, to run parallelly.
!
! !REVISION HISTORY:
!  2019-04-06 by NSCCJN @ Jinan.
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      i,j,n,             &! loop indices
      ibad,iead,         &! beginning and ending index of
      jbad,jead,         &!  physical domain for advu
      ibeg,iend,         &! beginning and ending index of
      jbeg,jend,         &!  physical domain for hdiffu	  
...
!!   real (r8) ::          &
!!      adcc                  ! scalar central weight for advu
...
!!      utmp, vtmp
   ...
   real (r8), dimension(nx_block,ny_block,4) :: &
      UVEWNS        ! for momen flux velocities across E,W,N,S faces
!!      CEWNS           ! for CN,CS,CE,CW
...
!-----------------------------------------------------------------------
  ...
  iparam(3)=k
  iparam(4)=km
  iparam(5)=this_block%ib
  iparam(6)=this_block%ie
  iparam(7)=this_block%jb
  iparam(8)=this_block%je
  iparam(9)=0
  iparam(10)=0
  iparam(11)=0
  if(partial_bottom_cells) iparam(9)=1
  if (mix_pass /= 1) then
     if (accumulate_tavg_now(tavg_UEU) .or. accumulate_tavg_now(tavg_UEV) .or. &
       accumulate_tavg_now(tavg_UEU) .or. accumulate_tavg_now(tavg_UEV) .or. &
       accumulate_tavg_now(tavg_UEU) .or. accumulate_tavg_now(tavg_UEV)) then
       iparam(10)=1
     endif
  endif
  if (lvariable_hmixu) iparam(11)=1
  iparam(12)=my_task
  ...
  rparam(9)=dz2r(k)
     ...
     call advuhdiffu_c(iparam,rparam,uuu(:,:,k),vvv(:,:,k),dxu(:,:,bid),dyu(:,:,bid),&
               dzu(:,:,k,bid),uvewns,wukb,wuk,uarea_r(:,:,bid),&
               luk,lvk,kxyu(:,:,:,bid),&
               duc(:,:,bid),dum(:,:,bid),&
               dun(:,:,bid),dus(:,:,bid),due(:,:,bid),duw(:,:,bid),&
               dmc(:,:,bid),dmn(:,:,bid),dms(:,:,bid),&
               dme(:,:,bid),dmw(:,:,bid),umixk,vmixk,&
               hduk,hdvk,amf(:,:,bid),kmu(:,:,bid) )
...
!----------------------------------------------------------------------
   ...
   if (mix_pass /= 1) then
    ...
    if(accumulate_tavg_now(tavg_UEU) .or. accumulate_tavg_now(tavg_UEV) .or.&
       accumulate_tavg_now(tavg_VNU) .or. accumulate_tavg_now(tavg_VNV) .or.&
       accumulate_tavg_now(tavg_WTU) .or. accumulate_tavg_now(tavg_WTV) ) then

      if (partial_bottom_cells) then
#ifndef NOATH 
         FUE =  uvewns(:,:,1)*p5*UAREA_R(:,:,bid)/DZU(:,:,k,bid)
#else
      ...
      do j=jbeg,jend
      do i=ibeg,iend
         WORK1(i,j) = FUE(i,j)*(UUU(i,j,k) + UUU(i+1,j,k))
      end do
      end do
      call accumulate_tavg_field(WORK1,tavg_UEU,bid,k)

      do j=jbeg,jend
      do i=ibeg,iend
         WORK2(i,j) = FUE(i,j)*(VVV(i  ,j,k) + VVV(i+1,j,k))
      end do
      end do
      call accumulate_tavg_field(WORK2,tavg_UEV,bid,k)

      if (partial_bottom_cells) then

#ifndef NOATH
         FVN =  uvewns(:,:,3)*p5*UAREA_R(:,:,bid)/DZU(:,:,k,bid)
#else
      ...
      do j=jbeg,jend
      do i=ibeg,iend
         WORK3(i,j) = FVN(i,j)*(UUU(i,j  ,k) + UUU(i,j+1,k))
      end do
      end do
      call accumulate_tavg_field(WORK3,tavg_VNU,bid,k)

      do j=jbeg,jend
      do i=ibeg,iend
         WORK4(i,j) = FVN(i,j)*(VVV(i,j  ,k) + VVV(i,j+1,k))
      end do
      end do
      call accumulate_tavg_field(WORK4,tavg_VNV,bid,k)

      if (k == 1) then
         WORK5 = dzr(k)*WUK*UUU(:,:,k)
      else
         WORK5 = dz2r(k)*WUK*(UUU(:,:,k) + UUU(:,:,k-1))
      endif
      call accumulate_tavg_field(WORK5,tavg_WTU,bid,k)

      if (k == 1) then
         WORK6 = dzr(k)*WUK*VVV(:,:,k)
      else
         WORK6 = dz2r(k)*WUK*(VVV(:,:,k) + VVV(:,:,k-1))
      endif
      call accumulate_tavg_field(WORK6,tavg_WTV,bid,k)

   WUK = WUKB   ! bottom value becomes top value for next pass
    endif !endof accum_tavg_now
  ...
  ! WUK = WUKB   ! bottom value becomes top value for next pass
   ...
   if(my_task == 0) WRITE (*,*) 'step out from hmix_del4: '
#endif
...
!EOC
 ...
 end subroutine advuhdiffu_del4
...
!loop16
  ...
  !   call getlooptime(finish)            
  !     if (my_task == master_task) then    
  !               WRITE (*,*) 'In hmix_del4, loop16 a costs : ',finish-start,' (us);' 
  !     endif   
 
  !     call getlooptime(start)       
  !     call loop16m_fun_sw(jbegin,jend,ibegin,iend,&    
  !                array_x,array_y,array_z,bid,&   
  !                k,&      
  !                DTN,DTS,DTE,DTW,DZT,&              
  !                CN,CS,CE,CW)  
   !    call getlooptime(finish)        
   !    if (my_task == master_task) then                
   !              WRITE (*,*) 'In hmix_del4, loop16  costs : ',finish-start,' (us);' 
   !   endif   
 ...
 !     call getlooptime(finish) 
 !     if (my_task == master_task) then  
 !        WRITE (*,*) 'In hmix_del4, loop16a1  costs : ',finish-start,' (us);' 
 !     endif   
     do j=this_block%jb-1,this_block%je+1  
     do i=this_block%ib-1,this_block%ie+1  
     if(k>KMT(i,j,bid)) then 
       ...
       if ( k > KMT(i-1,j,bid)) then  
        ...
        CC(i,j)=-(CN(i,j)+CS(i,j)+CE(i,j)+CW(i,j)) 
...
!     !    WRITE (*,*) 'In hmix_del4, loop17 a  costs :'
         end do
         end do
!      WRITE (*,*) 'In hmix_del4, loop17 a  '
!    call getlooptime(start)   
 !    call loop17am_fun_sw(jbegin,jend,ibegin,iend,&
 !                      array_x,array_y,array_z,n,&
 !                      k,bid,&
 !                      AHF,D2TK,TMIX,&
 !                      CC,CN,CS,CE,CW)

    !!         call loop17m_fun_sw(jbegin,jend,ibegin,iend,&  
     !!                    array_x,array_y,array_z,n,&
      !!                   k,bid,ah,&
       !!                  D2TK,AHF,TMIX,HDTK,&
        !!                 CC,CN,CS,CE,CW) 
...
!    call getlooptime(finish) 
!     if (my_task == master_task) then    
!            WRITE (*,*) 'In hmix_del4, loop17 a costs : ',finish-start,' (us);'
!      endif 
!     WRITE (*,*) 'In hmix_del4, loop17 b  '  
    HDTK(:,:,n) = c0  
     jbegin=this_block%jb   
     jend=this_block%je 
     ibegin=this_block%ib   
     iend=this_block%ie    
     array_x=nx_block    
     array_y=ny_block   
!!     array_z=nblocks_clinic 
     array_z=km   
!    if (my_task == master_task) then   
!         WRITE (*,*) 'In hmix_del4, loop17 a ' 
!    endif
        
 !    call loop17bm_fun_sw(jbegin,jend,ibegin,iend,& 
 !                      array_x,array_y,array_z,n,&  
 !                      k,ah,&           
 !                      D2TK,HDTK,&            
 !                      CC,CN,CS,CE,CW) 
...
!      do i=this_block%ib-1,this_block%ie+1
!     if(my_Task==master_task) then
!      if(ABS((v1_slave(i,j,n)-HDTK(i,j,n))/HDTK(i,j,n))>0.0000000000000001) then
!       print*,"loop07_diff_CN=",ABS(v1_slave(i,j,n)-HDTK(i,j,n))/HDTK(i,j,n),v1_slave(i,j,n),HDTK(i,j,n),i,j,n
 !   endif
 !  endif
 ! enddo
 ! enddo
...
!                         CC,CN,CS,CE,CW) 
         ...
         do i=this_block%ib-1,this_block%ie+1
            ...
            D2TK(i,j) = CC(i,j)*TMIX(i  ,j  ,k,n) +      &
                        ...
                        CE(i,j)*TMIX(i+1,j  ,k,n) +      &
                        CW(i,j)*TMIX(i-1,j  ,k,n)
      !      WRITE (*,*) 'In hmix_del4, loop17 b  costs :'          
         end do
         end do
         do j=this_block%jb,this_block%je                
         do i=this_block%ib,this_block%ie            
...
!-----------------------------------------------------------------------
 ...
 !                          CE(i,j)*D2TK(i+1,j  ) +     &
 !                          CW(i,j)*D2TK(i-1,j  ))
    !      WRITE (*,*) 'In hmix_del4, loop17 c  costs :'          
 !     end do
 !     end do
!  call getlooptime(start)
...
!           call getlooptime(start)              
 ...
 !                WRITE (*,*) 'In hmix_del4, loop17 d  costs :'          
            enddo
            enddo
  !          call getlooptime(start)  
  ...
  !         call getlooptime(finish)
  !           if (my_task == master_task) then              
  !            WRITE (*,*) 'In hmix_del4, loop17 d  costs : ',finish-start,'(us);'   
  !          endif
...
!           call getlooptime(start) 
     ...
     !             WRITE (*,*) 'In hmix_del4, loop17 e  costs :'          
            enddo
            enddo
 !          call getlooptime(finish)
 !          if (my_task == master_task) then         
 !                    WRITE (*,*) 'In hmix_del4, loop17 e  costs :',finish-start,'(us);'        
 !            endif
...
! call getlooptime(finish)
! if (my_task == master_task) then   
!      WRITE (*,*) 'In hmix_del4, loop17  b costs : ',finish-start,' (us);'        
!endif 
