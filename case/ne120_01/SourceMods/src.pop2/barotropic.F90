...
!   use distribution, only: 
   ...
   use timers
   ! use step_mod, only: timer_3dupdtrop
   ...
   do iblock = 1,nblocks_clinic
     ...
     !!120us
      if (leapfrogts) then               ! leapfrog
        ...
        ! call barotropic_loop4(iblock,curtime,oldtime,WORK3,WORK4,ZX,ZY,GRADPX,GRADPY,&
        !                      & c2dtp,gamma,c1)
      ...
      elseif (mix_pass == 1 .or. f_euler_ts) then   ! matsuno 1st pass
       ! call barotropic_loop5(iblock,curtime,WORK3,WORK4,ZX,ZY,GRADPX,GRADPY,&
       !                       & c2dtp)
       ...
       ! call barotropic_loop6(iblock,newtime,curtime,WORK3,WORK4,ZX,ZY,GRADPX,GRADPY,&
       !                       & c2dtp,theta,c1)
...
!-----------------------------------------------------------------------
    !! 11us
     ...
     !!!    call barotropic_loop7(WORK1,WORK2)  !slower
...
!-----------------------------------------------------------------------
    ...
    !debug
      !    call barotropic_loop8(iblock,oldtime,WORK1,WORK2,UH,VH,UBTROP,VBTROP,FCOR,&
      !                        & c2dtp,beta,c1)
     ...
     !debug
      !   call barotropic_loop9(iblock,oldtime,UH,VH,UBTROP,VBTROP)
!
...
!-----------------------------------------------------------------------
      ...
      if (leapfrogts) then               ! leapfrog
      !    call barotropic_loop10(iblock,oldtime,beta,HU)
      ...
      elseif (mix_pass == 1 .or. f_euler_ts) then   ! matsuno 1st pass
       !    call barotropic_loop10(iblock,curtime,beta,HU) 
       ...
       !   call barotropic_loop10(iblock,newtime,beta,HU) 
...
!1
    ...
    !   param1=1/(beta*c2dtp*dtp*grav)
    !     param2=1/(beta*c2dtp)
    !     call barotropic_loop11(iblock,curtime,diagonalCorrection,TAREA,CALCT,RHS,PSURF,FW,dtp,grav)
    !      call barotropic_loop11(iblock,curtime,diagonalCorrection,TAREA,CALCT,RHS,PSURF,FW,&
    !                    & param1,param2)
         diagonalCorrection(:,:) =                                     & 
...
!         call barotropic_loop12(iblock,curtime,diagonalCorrection,TAREA,CALCT,RHS,PSURF,dtp,grav)

!         call barotropic_loop12(iblock,curtime,diagonalCorrection,TAREA,CALCT,RHS,PSURF ,&
!                        & param1)
         diagonalCorrection(:,:) =                                     &
                        merge(TAREA(:,:,iblock)/(beta*c2dtp*dtp*grav), &
                              c0,CALCT(:,:,iblock))
...
!-----------------------------------------------------------------------
  ...
  !    call barotropic_loop4(nblocks_clinic,newtime,PCHECK,PSURF,CHECKER)
   ...
   ! call timer_start(timer_3dupdtrop)
   !$OMP END PARALLEL DO
   ...
   ! call timer_stop(timer_3dupdtrop)
...
!-----------------------------------------------------------------------
   ...
   if (sfc_layer_type == sfc_layer_varthick) then
    ...
    !  call barotropic_loop1(nblocks_clinic,newtime,PCHECK,PSURF,CHECKER)
      do iblock = 1,nblocks_clinic
...
!-----------------------------------------------------------------------
                                ! 1
      if (sfc_layer_type == sfc_layer_varthick) then
     !  call barotropic_loop2(iblock,newtime,PSURF,CONSTNT,CHECKER,rcheck,xcheck,rconst)
...
!-----------------------------------------------------------------------
      ...
      if (leapfrogts) then               ! leapfrog
     !    call barotropic_loop3(iblock,newtime,oldtime)
      ...
      elseif (mix_pass == 1 .or. f_euler_ts) then  ! matsuno 1st pass
     !   call barotropic_loop3_1(iblock,newtime,curtime)
         UBTROP(:,:,newtime,iblock) = UH(:,:,iblock) -                &
                             beta*c2dtp*(GRADPX(:,:,newtime,iblock) - &
     ...
     !   call barotropic_loop3_2(iblock,newtime,WORKX,WORKY)
         UBTROP(:,:,newtime,iblock) = UH(:,:,iblock) -                &
                             beta*c2dtp*(GRADPX(:,:,newtime,iblock) - &
                                         WORKX(:,:,iblock))
         VBTROP(:,:,newtime,iblock) = VH(:,:,iblock) -                &
