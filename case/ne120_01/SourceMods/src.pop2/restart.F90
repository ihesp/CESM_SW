...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(:,:,:,:), allocatable :: &
      TEMP_DATA3           ! temp array for reading T,S data
   real (r8), dimension(:,:,:), allocatable :: &
      TEMP_DATA2           ! temp array for reading T,S data
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
   ...
   allocate(TEMP_DATA2(nx_block,ny_block,max_blocks_clinic))
   ...
   if (liceform) then
      ...
      allocate(TEMP_DATA3(nx_block,ny_block,km,max_blocks_clinic))
...
!-----------------------------------------------------------------------
   ...
   call data_set (restart_file, 'read', UBTROP_CUR)
      do iblock=1,nblocks_clinic
         UBTROP(:,:,curtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   call data_set (restart_file, 'read', UBTROP_OLD)
      do iblock=1,nblocks_clinic
         UBTROP(:,:,oldtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   call data_set (restart_file, 'read', VBTROP_CUR)
      do iblock=1,nblocks_clinic
         VBTROP(:,:,curtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   call data_set (restart_file, 'read', VBTROP_OLD)
      do iblock=1,nblocks_clinic
         VBTROP(:,:,oldtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   call data_set (restart_file, 'read', PSURF_CUR)
      do iblock=1,nblocks_clinic
         PSURF(:,:,curtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   call data_set (restart_file, 'read', PSURF_OLD)
      do iblock=1,nblocks_clinic
         PSURF(:,:,oldtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   call data_set (restart_file, 'read', GRADPX_CUR)
      do iblock=1,nblocks_clinic
         GRADPX(:,:,curtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   call data_set (restart_file, 'read', GRADPX_OLD)
      do iblock=1,nblocks_clinic
         GRADPX(:,:,oldtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   call data_set (restart_file, 'read', GRADPY_CUR)
      do iblock=1,nblocks_clinic
         GRADPY(:,:,curtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   call data_set (restart_file, 'read', GRADPY_OLD)
      do iblock=1,nblocks_clinic
         GRADPY(:,:,oldtime,iblock) = TEMP_DATA2(:,:,iblock)
      end do
   ...
   call data_set (restart_file, 'read', UVEL_CUR)
      do iblock=1,nblocks_clinic
         UVEL(:,:,:,curtime,iblock) = TEMP_DATA3(:,:,:,iblock)
      end do
   call data_set (restart_file, 'read', UVEL_OLD)
      do iblock=1,nblocks_clinic
         UVEL(:,:,:,oldtime,iblock) = TEMP_DATA3(:,:,:,iblock)
      end do
   call data_set (restart_file, 'read', VVEL_CUR)
      do iblock=1,nblocks_clinic
         VVEL(:,:,:,curtime,iblock) = TEMP_DATA3(:,:,:,iblock)
      end do
   call data_set (restart_file, 'read', VVEL_OLD)
      do iblock=1,nblocks_clinic
         VVEL(:,:,:,oldtime,iblock) = TEMP_DATA3(:,:,:,iblock)
      end do

   do n=1,2
      call data_set (restart_file, 'read', TRACER_CUR(n))
      do iblock=1,nblocks_clinic
         TRACER(:,:,:,n,curtime,iblock) = TEMP_DATA3(:,:,:,iblock)
      end do
      call data_set (restart_file, 'read', TRACER_OLD(n))
      do iblock=1,nblocks_clinic
         TRACER(:,:,:,n,oldtime,iblock) = TEMP_DATA3(:,:,:,iblock)
      end do
   end do
     deallocate(TEMP_DATA2)
     deallocate(TEMP_DATA3)
...
!-----------------------------------------------------------------------
      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock) = UBTROP(:,:,curtime,iblock)  
      end do
   call data_set (restart_file, 'write', UBTROP_CUR)

      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock) = UBTROP(:,:,oldtime,iblock)  
      end do
   call data_set (restart_file, 'write', UBTROP_OLD)

      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock) = VBTROP(:,:,curtime,iblock)  
      end do
   call data_set (restart_file, 'write', VBTROP_CUR)

      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock) = VBTROP(:,:,oldtime,iblock) 
      end do
   call data_set (restart_file, 'write', VBTROP_OLD)

      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock)= PSURF(:,:,curtime,iblock)  
      end do
   call data_set (restart_file, 'write', PSURF_CUR)

      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock)=PSURF(:,:,oldtime,iblock)  
      end do
   call data_set (restart_file, 'write', PSURF_OLD)

      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock)=GRADPX(:,:,curtime,iblock)  
      end do
   call data_set (restart_file, 'write', GRADPX_CUR)

      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock)=GRADPX(:,:,oldtime,iblock)  
      end do
   call data_set (restart_file, 'write', GRADPX_OLD)

      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock)=GRADPY(:,:,curtime,iblock)  
      end do
   call data_set (restart_file, 'write', GRADPY_CUR)

      do iblock=1,nblocks_clinic
         TEMP_DATA2(:,:,iblock)=GRADPY(:,:,oldtime,iblock)  
      end do
   ...
   if (liceform) then
      ...
      do iblock=1,nblocks_clinic
         TEMP_DATA3(:,:,:,iblock)=UVEL(:,:,:,curtime,iblock)  
      end do
   call data_set (restart_file, 'write', UVEL_CUR)

      do iblock=1,nblocks_clinic
         TEMP_DATA3(:,:,:,iblock)=UVEL(:,:,:,oldtime,iblock)  
      end do
   call data_set (restart_file, 'write', UVEL_OLD)

      do iblock=1,nblocks_clinic
         TEMP_DATA3(:,:,:,iblock)=VVEL(:,:,:,curtime,iblock)  
      end do
   call data_set (restart_file, 'write', VVEL_CUR)

      do iblock=1,nblocks_clinic
         TEMP_DATA3(:,:,:,iblock)=VVEL(:,:,:,oldtime,iblock)  
      end do
   call data_set (restart_file, 'write', VVEL_OLD)
   do n=1,nt

      do iblock=1,nblocks_clinic
         TEMP_DATA3(:,:,:,iblock)=TRACER(:,:,:,n,curtime,iblock)  
      end do
      call data_set (restart_file, 'write', TRACER_CUR(n))
      do iblock=1,nblocks_clinic
         TEMP_DATA3(:,:,:,iblock)=TRACER(:,:,:,n,oldtime,iblock)  
      end do
