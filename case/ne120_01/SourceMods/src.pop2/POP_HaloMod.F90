...
!!!   allocate(halo%sendAddr_zy(1,1,1),&
!!!            halo%sendAddr_zy(1,1,1),stat =istat)
...
!!!   deallocate(halo%sendAddr_zy, halo%recvAddr_zy,         &
!!!              stat = istat)
...
!!!
!!!   nullify(halo%sendAddr_zy)
!!!   nullify(halo%recvAddr_zy)
...
!!!ZYU
 ...
 subroutine POP_HaloUpdate_zy(array, halo,                    &
...
!-----------------------------------------------------------------------
 ...
 !!     rcvStatus
   ...
   integer (POP_i4), dimension(16):: &
      sndRequest_zy,       &! MPI status flags
      rcvRequest_zy         ! MPI status flags

   integer (POP_i4), dimension(MPI_STATUS_SIZE,16) :: &
      sndStatus_zy,       &! MPI status flags
      rcvStatus_zy         ! MPI status flags


 !!  integer (POP_i4), dimension(64) :: sndRequest,rcvRequest,sndStatus,rcvStatus 
!!!   common  sndRequest,rcvRequest
!-----------------------------------------------------------------------
!
...
!   if(POP_myTask.eq.0 .and.times.lt.10000 ) then
!     write(*,*)"value:",MPI_STATUS_SIZE,halo%numMsgSend,halo%numMsgRecv
!     times=times+1
!   endif
...
!!     allocate(sndStatus(MPI_STATUS_SIZE,halo%numMsgSend), &
!!             rcvStatus(MPI_STATUS_SIZE,halo%numMsgRecv), stat=ierr)
...
!-----------------------------------------------------------------------
   ...
   do nmsg=1,halo%numMsgRecv
      ...
      call MPI_IRECV(bufRecvR8(1:msgSize,nmsg), msgSize, POP_mpiR8, &
                     ...
                     halo%communicator, rcvRequest_zy(nmsg), ierr)
   end do
...
!!    count=0
!!    do while(count<halo%numMsgRecv)
!!      call MPI_WAITany(halo%numMsgRecv, rcvRequest_zy, nmsg, rcvStatus_zy, ierr)
!!      count=count+1
!!    end do


    call MPI_WAITALL(halo%numMsgRecv, rcvRequest_zy, rcvStatus_zy, ierr)
...
!    count=0
!    do while(count<halo%numMsgSend)
!      call MPI_WAITany(halo%numMsgSend, sndRequest_zy, nmsg, rcvStatus, ierr)
!      count=count+1
!    end do

   call MPI_WAITALL(halo%numMsgSend, sndRequest_zy, sndStatus_zy, ierr)

#if defined(NEW)
   deallocate(sndRequest, rcvRequest, sndStatus, rcvStatus, stat=ierr)
   flag=1
#endif
!!!   deallocate(sndStatus, rcvStatus, stat=ierr)
...
!EOC
 ...
 end subroutine POP_HaloUpdate_zy
...
!!  if(POP_myTask ==5)then            
!!         write(*,*)"1,2,3,4",halo%numMsgSend,halo%numMsgRecv,MPI_STATUS_SIZE
!!  endif
...
!!!call getlooptime(st03)

!!!    if(POP_myTask ==5)then
!!!              write(*,*)"before- pre  wait all cost=",st02-st01,"us; recv cost ",st03-st02,"us"
!!!     endif
...
!-----------------------------------------------------------------------
  ...
  !  count=0
  !  do while(count<halo%numMsgRecv)
  !    call MPI_WAITany(halo%numMsgRecv, rcvRequest_zy, nmsg, rcvStatus, ierr)
  !    count=count+1
  !  end do
...
!!!    if(POP_myTask ==5)then
!!!              write(*,*)"wait all cost=",st02-st01,"us;"
!!!     endif
...
!!!    if(POP_myTask ==5)then
!!!              write(*,*)"after wait all cost=",st02-st01,"us;"
!!!     endif
...
!!!    if(POP_myTask ==5)then
!!!              write(*,*)"end wait all  cost=",st02-st01,"us;"
!!!     endif
!!!#if defined(aaa)
...
! !INTERFACE:
   ...
   subroutine POP_HaloMsgCreate_zy(halo, srcBlock, srcProc, srcLocalID, &
...
!EOC
   ...
   end subroutine POP_HaloMsgCreate_zy
!-----------------------------------------------------------------------
!EOC
