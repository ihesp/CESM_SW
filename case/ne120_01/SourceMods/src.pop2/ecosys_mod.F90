...
!   use passive_tracer_tools,only : forcing_monthly_every_ts
   ...
   use ecosys_restore_mod, only : init
   use ecosys_restore_mod, only : initialize_restoring_timescale
   use ecosys_restore_mod, only : read_restoring_fields
   use ecosys_restore_mod, only : restore_variable
   use ecosys_restore_mod, only : accumulate_tavg
   use ecosys_restore_mod, only : define_tavg_fields
#ifdef CCSMCOUPLED
...
!        ind_name_table)
   call Init(ecosys_restore,nml_filename, nml_in, &
        ind_name_table)
...
!   call ecosys_restore%define_tavg_fields()
   call define_tavg_fields(ecosys_restore)
...
!-----------------------------------------------------------------------
   ...
   do n = 1, ecosys_tracer_cnt
      ...
      call restore_variable(ecosys_restore,tracer_index=n, &
      ...
      call accumulate_tavg(ecosys_restore,tracer_index=n, &
...
!-----------------------------------------------------------------------
   ...
   do auto_ind = 1, autotroph_cnt
      ...
      where (tracer_local(:, :, fe_ind) < cks * autotrophs(auto_ind)%kFe)
         gQfe(:,:,auto_ind) = &
            max((gQfe(:,:,auto_ind) * tracer_local(:, :, fe_ind)/ (cks * autotrophs(auto_ind)%kFe)), &
                autotrophs(auto_ind)%gQfe_min)
      end where

      if (autotrophs(auto_ind)%Si_ind > 0) then
         gQsi(:,:,auto_ind) = gQsi_0
         where ((tracer_local(:, :, fe_ind) < cksi * autotrophs(auto_ind)%kFe) .and. (tracer_local(:, :, fe_ind)> c0) .and. &
                (tracer_local(:, :, sio3_ind) > (cksi * autotrophs(auto_ind)%kSiO3)))
            gQsi(:,:,auto_ind) = min((gQsi(:,:,auto_ind) * cksi * autotrophs(auto_ind)%kFe /tracer_local(:, :, fe_ind)), gQsi_max)
         end where

         where (tracer_local(:, :, fe_ind) == c0)
            gQsi(:,:,auto_ind) = gQsi_max
         end where

         where (tracer_local(:, :, sio3_ind) < (cksi * autotrophs(auto_ind)%kSiO3))
            gQsi(:,:,auto_ind) = max((gQsi(:,:,auto_ind) * tracer_local(:, :, sio3_ind) / (cksi * autotrophs(auto_ind)%kSiO3)), &
                                          gQsi_min)
         end where
      endif
   ...
   if (accumulate_tavg_now(tavg_CO3) .or. &
      ...
      do j = 1,ny_block
         ...
         call comp_CO3terms(bid, j, k, LAND_MASK(:,j,bid) .and. k <= KMT(:,j,bid), .true., &
                            TEMP(:,j),SALT(:,j),tracer_local(:,j,dic_ind),tracer_local(:,j,alk_ind),&
                            tracer_local(:,j,po4_ind),tracer_local(:,j,sio3_ind), &
         ...
         if (lalt_co2_terms) then
            ...
            call comp_CO3terms(bid, j, k, LAND_MASK(:,j,bid) .and. k <= KMT(:,j,bid), .false., &
                               TEMP(:,j), SALT(:,j), tracer_local(:, j, dic_alt_co2_ind),tracer_local(:, j, alk_ind),tracer_local(:,j, po4_ind), tracer_local(:,j, sio3_ind), &
...
!-----------------------------------------------------------------------
   ...
   do auto_ind = 1, autotroph_cnt
      VNO3(:,:,auto_ind) = (tracer_local(:, :, no3_ind) / autotrophs(auto_ind)%kNO3) / &
         (c1+ (tracer_local(:, :, no3_ind) / autotrophs(auto_ind)%kNO3) + (tracer_local(:, :, nh4_ind) / autotrophs(auto_ind)%kNH4))
      VNH4(:,:,auto_ind) = (tracer_local(:, :, nh4_ind) / autotrophs(auto_ind)%kNH4) / &
         (c1 + (tracer_local(:, :, no3_ind) / autotrophs(auto_ind)%kNO3) + (tracer_local(:, :, nh4_ind) / autotrophs(auto_ind)%kNH4))
      ...
      VFe = tracer_local(:, :, fe_ind) / (tracer_local(:, :, fe_ind) + autotrophs(auto_ind)%kFe)
      call accumulate_tavg_field(VFe, tavg_Fe_lim(auto_ind),bid,k)

      f_nut = min(VNtot(:,:,auto_ind), VFe)

      VPO4 = (tracer_local(:, :, po4_ind)/ autotrophs(auto_ind)%kPO4) / &
         (c1 + (tracer_local(:, :, po4_ind)/ autotrophs(auto_ind)%kPO4) + (tracer_local(:, :, dop_ind) / autotrophs(auto_ind)%kDOP))
      VDOP = (tracer_local(:, :, dop_ind)/ autotrophs(auto_ind)%kDOP) / &
         (c1 + (tracer_local(:, :, po4_ind)/ autotrophs(auto_ind)%kPO4) + (tracer_local(:, :, dop_ind)/ autotrophs(auto_ind)%kDOP))
      VPtot = VPO4 + VDOP
      ...
      if (autotrophs(auto_ind)%kSiO3 > c0) then
         VSiO3 = tracer_local(:, :, sio3_ind) / (tracer_local(:, :, sio3_ind) + autotrophs(auto_ind)%kSiO3)
...
!-----------------------------------------------------------------------
   ...
   DOC_remin  = tracer_local(:, :, doc_ind)  * DOC_reminR
   DON_remin  = tracer_local(:, :, don_ind)  * DON_reminR
   DOFe_remin = tracer_local(:, :, dofe_ind) * DOFe_reminR
   DOP_remin  = tracer_local(:, :, dop_ind)  * DOP_reminR
...
!-----------------------------------------------------------------------
   ...
   where (PAR_avg > 1.0_r8)
      DONr_remin = tracer_local(:, :, donr_ind) * DONr_reminR
      DOPr_remin = tracer_local(:, :, dopr_ind) * DOPr_reminR
   elsewhere
      DONr_remin = tracer_local(:, :, donr_ind) * (c1/(365.0_r8*670.0_r8)) * dps  ! 1/670 yrs
      DOPr_remin = tracer_local(:, :, dopr_ind) * (c1/(365.0_r8*460.0_r8)) * dps  ! 1/460 yrs
...
!-----------------------------------------------------------------------
   ...
   where (tracer_local(:, :, fe_ind) > Fe_scavenge_thres1) &
      Fe_scavenge_rate = Fe_scavenge_rate + &
                         (tracer_local(:, :, fe_ind) - Fe_scavenge_thres1) * fe_max_scale2

   Fe_scavenge = yps * tracer_local(:, :, fe_ind) * Fe_scavenge_rate
   ...
   call compute_particulate_terms(k, POC, P_CaCO3, P_SiO2, dust, P_iron, &
                                  QA_dust_def, TEMP, tracer_local(:, :, o2_ind), tracer_local(:, :,no3_ind), &
...
!-----------------------------------------------------------------------
   ...
   where (PAR_out(:,:,bid) < parm_nitrif_par_lim)
      NITRIF = parm_kappa_nitrif * tracer_local(:, :, nh4_ind)
...
!-----------------------------------------------------------------------
   ...
   WORK1 = ((parm_o2_min + parm_o2_min_delta) - tracer_local(:, :, o2_ind)) / parm_o2_min_delta
   WORK1 = min(max(WORK1,c0),c1)

   WORK1 = merge(c0, WORK1, tracer_local(:, :, no3_ind) == c0)
...
!-----------------------------------------------------------------------
   ...
   WORK1 = (tracer_local(:, :, o2_ind) - parm_o2_min) / parm_o2_min_delta
   WORK1 = min(max(WORK1,c0),c1)
...
!-----------------------------------------------------------------------
   ...
   if (lexport_shared_vars) then
      DIC_loc_fields(:,:,bid) = tracer_local(:, :, dic_ind)
      DOC_loc_fields(:,:,bid) = tracer_local(:, :, doc_ind)
      O2_loc_fields(:,:,bid) = tracer_local(:, :, o2_ind)
      NO3_loc_fields(:,:,bid) = tracer_local(:, :, no3_ind)
...
!-----------------------------------------------------------------------
   ...
   call initialize_restoring_timescale(ecosys_restore,nml_filename, nml_in, zt)
...
!-----------------------------------------------------------------------
   ...
   call read_restoring_fields(ecosys_restore,LAND_MASK)
