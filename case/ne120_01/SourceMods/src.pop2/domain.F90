...
! timing_var_def added by zengyh 2018-11-30:
#if defined(POPTIMEJN) || defined(POPCHKJN) 
    real(8)::start_gtim,stop_gtim
#endif
...
!----------------------------------------------------------------------
   ...
   if (my_task < distrb_clinic%nprocs .and. associated(blocks_clinic)) then
...
!------- added by zengyh 2018-11-30:----
#if defined(POPTIMEJN) || defined(POPCHKJN)    
  call getlooptime(start_gtim)
#endif  
...
!!   call MPI_ALLREDUCE(nblocks_tmp_clinic, nblocks_max_clinic, 1, MPI_INTEGER, MPI_MAX, MPI_COMM_OCN, ierr)
   nblocks_max_clinic = POP_GlobalMaxval(nblocks_tmp_clinic, POP_distrbClinic, errorCode)
!!   nblocks_max_clinic = POP_GlobalMaxval(nblocks_tmp_clinic, distrb_clinic, errorCode)


!------- added by zengyh 2018-11-30:----
#if defined(POPTIMEJN) || defined(POPCHKJN)    
  call getlooptime(stop_gtim)
  if (my_task == master_task) then
     WRITE (*,*) 'mpi_allreduce of broad_scalar costs zengyh: ',stop_gtim-start_gtim,'(us);'
  endif
  ...
  call getlooptime(stop_gtim)
  if (my_task == master_task) then
     WRITE (*,*) 'initial broad_scalar costs zengyh: ',stop_gtim-start_gtim,'(us);'
  endif
#endif  
...
!!   call MPI_ALLREDUCE(nblocks_tmp, nblocks_max_tropic, 1, MPI_INTEGER, MPI_MAX, MPI_COMM_OCN, ierr)

   nblocks_max_tropic = POP_GlobalMaxval(nblocks_tmp, POP_distrbTropic, errorCode)

!------- added by zengyh 2018-11-30:----
#if defined(POPTIMEJN) || defined(POPCHKJN)    
  call getlooptime(stop_gtim)
  if (my_task == master_task) then
     WRITE (*,*) 'mpi_allreduce tropic of broad_scalar costs zengyh: ',stop_gtim-start_gtim,'(us);'
  endif
  ...
  call getlooptime(stop_gtim)
  if (my_task == master_task) then
     WRITE (*,*) 'initial tropic broad_scalar costs zengyh: ',stop_gtim-start_gtim,'(us);'
  endif
#endif  
