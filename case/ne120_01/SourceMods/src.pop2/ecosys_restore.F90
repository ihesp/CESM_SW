module ecosys_restore_mod
  ...
  use ecosys_restore_timescale_file, only : file_init
  use ecosys_restore_timescale_interp, only : ecosys_restore_timescale_interp_type
  use ecosys_restore_timescale_interp, only : interp_init

  implicit none

  private
  public :: init
  public :: read_restoring_fields
  public :: restore_variable
  public :: define_tavg_fields
  public :: accumulate_tavg
  public :: initialize_restoring_timescale
  private :: read_namelist
  private :: initialize_restore_read_vars

  type ,public:: restore_info
  ...
  type, public :: ecosys_restore_type
     logical(log_kind) :: restore_any_tracer ! true if we are restoring any tracer
     type(restore_info), allocatable :: tracers(:)
     ...
     type(ecosys_restore_timescale_file_type) :: timescale_file
     type(ecosys_restore_timescale_interp_type) :: timescale_interp
...
subroutine init(this, nml_filename, nml_in, ind_name_table)
  ...
  call initialize_restore_read_vars(this,restore_short_names, restore_filenames, &
...
subroutine initialize_restoring_timescale(this, nml_filename, nml_in, zt)
  ...
  if (this%restore_any_tracer) then
     ...
     !   call this%timescale_file%init(nml_filename, nml_in)
        call file_init(this%timescale_file,nml_filename, nml_in)
     else
     !   call this%timescale_interp%init(nml_filename, nml_in, zt)
        call interp_init(this%timescale_interp,nml_filename, nml_in, zt)
     end if
     if (my_task == master_task) then
...
!     associate(tracer => this%tracers(tracer_index))
       ...
       if (this%tracers(tracer_index)%restore) then
!!$          write(stdout, *) "Reading restore data for ", trim(tracer%restore_file_info%mod_varname)
          allocate(this%tracers(tracer_index)%data(nx_block, ny_block, km, max_blocks_clinic))

          call read_field(this%tracers(tracer_index)%restore_file_info%file_fmt, &
               this%tracers(tracer_index)%restore_file_info%filename, &
               this%tracers(tracer_index)%restore_file_info%file_varname, &
               this%tracers(tracer_index)%data)
                ...
                do j=1,ny_block
                   do i=1,nx_block
                      if (LAND_MASK(i,j,iblock) .and. (k.le.KMT(i,j,iblock))) then
                         this%tracers(tracer_index)%data(i,j,k,iblock) = this%tracers(tracer_index)%data(i,j,k,iblock) &
                                * this%tracers(tracer_index)%restore_file_info%scale_factor
                      else
                         this%tracers(tracer_index)%data(i,j,k,iblock) = c0
                      end if
                   end do
                end do
             end do
...
!       )
    ...
    if (this%tracers(tracer_index)%restore) then
       if (this%spatial_variability_from_file) then
          restore_data = this%timescale_file%restore_rtau(:, :, block_id) * &
               merge((this%tracers(tracer_index)%data(:, :, vert_level, block_id) - local_data), &
               c0, vert_level <= this%timescale_file%restore_max_level(:, :, block_id))
       else
          restore_data = (this%tracers(tracer_index)%data(:, :, vert_level, block_id) - local_data) * this%timescale_interp%inv_restoring_time_scale(vert_level)
       endif
    else
       restore_data = c0
    endif
