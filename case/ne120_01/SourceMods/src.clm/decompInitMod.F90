...
!  use mct_mod
!add by chenyuhu
  use mct_mod     , only : mct_gsMap_ngseg,mct_gsmap_init
  use decompMod
!
! !PUBLIC TYPES:
  implicit none
...
! !LOCAL VARIABLES:
    ...
    integer :: beg,end,llsize,gsize    ! used for gsmap init
...
!------------------------------------------------------------------------------
    ...
    llsize = end-beg+1
    gsize = lni * lnj
    call mct_gsMap_init(gsMap_lnd_gdc2glo, gindex, mpicom, comp_id, llsize, gsize)
    deallocate(gindex)
...
! !LOCAL VARIABLES:
    ...
    integer :: llsize,gsize        ! used for gsmap init
...
!------------------------------------------------------------------------------
    ...
    do k = 1,4
       ...
       llsize = end-beg+1
       gsize = num
       call mct_gsMap_init(gsMap, gindex, mpicom, comp_id, llsize, gsize)
