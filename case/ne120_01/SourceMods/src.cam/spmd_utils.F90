...
!========================================================================
  ...
  subroutine spmdinit( mpicom_atm )
    ...
    !==============changed by guoheng========
        !========================================
        !======only used for Sunway TaihuLight===
        integer :: nodenum ! node num of a process
        character(len=6)    :: nodename ! node name of a process
        integer :: superNodeId ! super node name for a process
        integer, dimension(:), allocatable    :: superNodeIds ! super node names
...
!====changed by guoheng=============
allocate ( superNodeIds(0:npes-1) )
!====end change
       ...
       proc_smp_map(0) = 0
!=======changed by guoheng=========
nodename = tmp_name(3:8)
read(nodename, *) nodenum ! get nodenum from nodename
superNodeId = nodenum / 256
superNodeIds(0) = superNodeId
!======end change
...
!             if (smp_names(j) .eq. trim(tmp_name)) then
              if ( superNodeIds(j) .eq. superNodeId) then
                proc_smp_map(i) = j
                done = .true.
             endif
             j = j + 1
...
!======changed by guoheng
superNodeIds(nsmps) = superNodeId
!======end change
