...
use linear_1d_operators, only: TriDiagDecomp,decomp_left_div
...
!
  ...
  use coords_1d, only: Coords1D,chen_Coords1D_section,&
...
!--------------------------------------------------------------------------
  ...
  call chen_Coords1D_section(p,[1,ncol],[ktop,kbot],section)
  call fin_vol_lu_decomp(decomp,dt,section, &
       coef_q_diff=egwdffi(:,ktop:kbot+1)*dpidz_sq(:,ktop:kbot+1))

  call chen_Coords1D_finalize(section)
...
!--------------------------------------------------------------------------
  ...
  call decomp_left_div(decomp,qnew(:,ktop:kbot))
