...
!
!      packed_array = xpack(packer,original_array)
!
...
!
!      call add_field(post_proc,p(final_array),p(packed_array))
!
...
subroutine micro_mg_cam_tend(state, ptend, dtime, pbuf)
   ...
   use micro_mg_data, only: MGPacker, MGPostProc, accum_null, accum_mean,&
                            xpack,pack_interface,xunpack,&
                            chen_new_MGPacker,chen_new_MGPostProc,&
                            add_field,accumulate,process_and_unpack,finalize
   ...
   type(MGPostProc) :: new_MGPostProc
  ...
  real(r8), pointer :: frzdep(:,:)
   ...
   call chen_new_MGPacker(psetcols, pver, mgcols, top_lev,packer)
   call chen_new_MGPostProc(packer,post_proc)
...
!   call add_field(post_proc,p(rate1cld), pckdptr)
   call add_field(post_proc,p(rate1cld), pckdptr)
   allocate(packed_tlat(mgncol,nlev))
!   call add_field(post_proc,p(tlat), p(packed_tlat))
   call add_field(post_proc,p(tlat), p(packed_tlat))
   allocate(packed_qvlat(mgncol,nlev))
!   call add_field(post_proc,p(qvlat), p(packed_qvlat))
   call add_field(post_proc,p(qvlat), p(packed_qvlat))
   allocate(packed_qctend(mgncol,nlev))
!   call add_field(post_proc,p(qcten), p(packed_qctend))
   call add_field(post_proc,p(qcten), p(packed_qctend))
   allocate(packed_qitend(mgncol,nlev))
   call add_field(post_proc,p(qiten), p(packed_qitend))
   allocate(packed_nctend(mgncol,nlev))
   call add_field(post_proc,p(ncten), p(packed_nctend))
   allocate(packed_nitend(mgncol,nlev))
   call add_field(post_proc,p(niten), p(packed_nitend))

   if (micro_mg_version > 1) then
      allocate(packed_qrtend(mgncol,nlev))
      call add_field(post_proc,p(qrten), p(packed_qrtend))
      allocate(packed_qstend(mgncol,nlev))
      call add_field(post_proc,p(qsten), p(packed_qstend))
      allocate(packed_nrtend(mgncol,nlev))
      call add_field(post_proc,p(nrten), p(packed_nrtend))
      allocate(packed_nstend(mgncol,nlev))
      call add_field(post_proc,p(nsten), p(packed_nstend))
      allocate(packed_umr(mgncol,nlev))
      call add_field(post_proc,p(umr), p(packed_umr))
      allocate(packed_ums(mgncol,nlev))
      call add_field(post_proc,p(ums), p(packed_ums))
   else if (micro_mg_sub_version == 0) then
      allocate(packed_am_evp_st(mgncol,nlev))
      call add_field(post_proc,p(am_evp_st), p(packed_am_evp_st))
   end if

   allocate(packed_prect(mgncol))
   call add_field(post_proc,p(prect), p(packed_prect))
   allocate(packed_preci(mgncol))
   call add_field(post_proc,p(preci), p(packed_preci))
   allocate(packed_nevapr(mgncol,nlev))
   call add_field(post_proc,p(nevapr), p(packed_nevapr))
   allocate(packed_evapsnow(mgncol,nlev))
   call add_field(post_proc,p(evapsnow), p(packed_evapsnow))
   allocate(packed_prain(mgncol,nlev))
   call add_field(post_proc,p(prain), p(packed_prain))
   allocate(packed_prodsnow(mgncol,nlev))
   call add_field(post_proc,p(prodsnow), p(packed_prodsnow))
   allocate(packed_cmeout(mgncol,nlev))
   call add_field(post_proc,p(cmeice), p(packed_cmeout))
   allocate(packed_qsout(mgncol,nlev))
   call add_field(post_proc,p(qsout), p(packed_qsout))
   allocate(packed_rflx(mgncol,nlev+1))
   call add_field(post_proc,p(rflx), p(packed_rflx))
   allocate(packed_sflx(mgncol,nlev+1))
   call add_field(post_proc,p(sflx), p(packed_sflx))
   allocate(packed_qrout(mgncol,nlev))
   call add_field(post_proc,p(qrout), p(packed_qrout))
   allocate(packed_qcsevap(mgncol,nlev))
   call add_field(post_proc,p(qcsevap), p(packed_qcsevap))
   allocate(packed_qisevap(mgncol,nlev))
   call add_field(post_proc,p(qisevap), p(packed_qisevap))
   allocate(packed_qvres(mgncol,nlev))
   call add_field(post_proc,p(qvres), p(packed_qvres))
   allocate(packed_cmei(mgncol,nlev))
   call add_field(post_proc,p(cmeiout), p(packed_cmei))
   allocate(packed_vtrmc(mgncol,nlev))
   call add_field(post_proc,p(vtrmc), p(packed_vtrmc))
   allocate(packed_vtrmi(mgncol,nlev))
   call add_field(post_proc,p(vtrmi), p(packed_vtrmi))
   allocate(packed_qcsedten(mgncol,nlev))
   call add_field(post_proc,p(qcsedten), p(packed_qcsedten))
   allocate(packed_qisedten(mgncol,nlev))
   call add_field(post_proc,p(qisedten), p(packed_qisedten))

   allocate(packed_pra(mgncol,nlev))
   call add_field(post_proc,p(prao), p(packed_pra))
   allocate(packed_prc(mgncol,nlev))
   call add_field(post_proc,p(prco), p(packed_prc))
   allocate(packed_mnuccc(mgncol,nlev))
   call add_field(post_proc,p(mnuccco), p(packed_mnuccc))
   allocate(packed_mnucct(mgncol,nlev))
   call add_field(post_proc,p(mnuccto), p(packed_mnucct))
   allocate(packed_msacwi(mgncol,nlev))
   call add_field(post_proc,p(msacwio), p(packed_msacwi))
   allocate(packed_psacws(mgncol,nlev))
   call add_field(post_proc,p(psacwso), p(packed_psacws))
   allocate(packed_bergs(mgncol,nlev))
   call add_field(post_proc,p(bergso), p(packed_bergs))
   allocate(packed_berg(mgncol,nlev))
   call add_field(post_proc,p(bergo), p(packed_berg))
   allocate(packed_melt(mgncol,nlev))
   call add_field(post_proc,p(melto), p(packed_melt))
   allocate(packed_homo(mgncol,nlev))
   call add_field(post_proc,p(homoo), p(packed_homo))
   allocate(packed_qcres(mgncol,nlev))
   call add_field(post_proc,p(qcreso), p(packed_qcres))
   allocate(packed_prci(mgncol,nlev))
   call add_field(post_proc,p(prcio), p(packed_prci))
   allocate(packed_prai(mgncol,nlev))
   call add_field(post_proc,p(praio), p(packed_prai))
   allocate(packed_qires(mgncol,nlev))
   call add_field(post_proc,p(qireso), p(packed_qires))
   allocate(packed_mnuccr(mgncol,nlev))
   call add_field(post_proc,p(mnuccro), p(packed_mnuccr))
   allocate(packed_pracs(mgncol,nlev))
   call add_field(post_proc,p(pracso), p(packed_pracs))
   allocate(packed_meltsdt(mgncol,nlev))
   call add_field(post_proc,p(meltsdt), p(packed_meltsdt))
   allocate(packed_frzrdt(mgncol,nlev))
   call add_field(post_proc,p(frzrdt), p(packed_frzrdt))
   allocate(packed_mnuccd(mgncol,nlev))
   call add_field(post_proc,p(mnuccdo), p(packed_mnuccd))
   allocate(packed_nrout(mgncol,nlev))
   call add_field(post_proc,p(nrout), p(packed_nrout))
   allocate(packed_nsout(mgncol,nlev))
   call add_field(post_proc,p(nsout), p(packed_nsout))

   allocate(packed_refl(mgncol,nlev))
   call add_field(post_proc,p(refl), p(packed_refl), -9999._r8)
   allocate(packed_arefl(mgncol,nlev))
   call add_field(post_proc,p(arefl), p(packed_arefl))
   allocate(packed_areflz(mgncol,nlev))
   call add_field(post_proc,p(areflz), p(packed_areflz))
   allocate(packed_frefl(mgncol,nlev))
   call add_field(post_proc,p(frefl), p(packed_frefl))
   allocate(packed_csrfl(mgncol,nlev))
   call add_field(post_proc,p(csrfl), p(packed_csrfl), -9999._r8)
   allocate(packed_acsrfl(mgncol,nlev))
   call add_field(post_proc,p(acsrfl), p(packed_acsrfl))
   allocate(packed_fcsrfl(mgncol,nlev))
   call add_field(post_proc,p(fcsrfl), p(packed_fcsrfl))

   allocate(packed_rercld(mgncol,nlev))
   call add_field(post_proc,p(rercld), p(packed_rercld))
   allocate(packed_ncai(mgncol,nlev))
   call add_field(post_proc,p(ncai), p(packed_ncai))
   allocate(packed_ncal(mgncol,nlev))
   call add_field(post_proc,p(ncal), p(packed_ncal))
   allocate(packed_qrout2(mgncol,nlev))
   call add_field(post_proc,p(qrout2), p(packed_qrout2))
   allocate(packed_qsout2(mgncol,nlev))
   call add_field(post_proc,p(qsout2), p(packed_qsout2))
   allocate(packed_nrout2(mgncol,nlev))
   call add_field(post_proc,p(nrout2), p(packed_nrout2))
   allocate(packed_nsout2(mgncol,nlev))
   call add_field(post_proc,p(nsout2), p(packed_nsout2))
   allocate(packed_freqs(mgncol,nlev))
   call add_field(post_proc,p(freqs), p(packed_freqs))
   allocate(packed_freqr(mgncol,nlev))
   call add_field(post_proc,p(freqr), p(packed_freqr))
   allocate(packed_nfice(mgncol,nlev))
   call add_field(post_proc,p(nfice), p(packed_nfice))
   if (micro_mg_version /= 1 .or. micro_mg_sub_version /= 0) then
      allocate(packed_qcrat(mgncol,nlev))
      call add_field(post_proc,p(qcrat), p(packed_qcrat), 1._r8)
   end if
   ...
   call add_field(post_proc,p(rel), p(packed_rel), &
        10._r8, accum_null)
   allocate(packed_rei(mgncol,nlev))
   call add_field(post_proc,p(rei), p(packed_rei), &
        25._r8, accum_null)
   allocate(packed_lambdac(mgncol,nlev))
   call add_field(post_proc,p(lambdac), p(packed_lambdac), &
        0._r8,accum_null)
   allocate(packed_mu(mgncol,nlev))
   call add_field(post_proc,p(mu), p(packed_mu), &
        0._r8,accum_null)
   allocate(packed_des(mgncol,nlev))
   call add_field(post_proc,p(des), p(packed_des), &
        0._r8,accum_null)
   allocate(packed_dei(mgncol,nlev))
   call add_field(post_proc,p(dei), p(packed_dei), &
        0._r8,accum_null)
   allocate(packed_prer_evap(mgncol,nlev))
   call add_field(post_proc,p(prer_evap), p(packed_prer_evap), &
        0._r8,accum_null)
   ...
   packed_relvar = xpack(packer,relvar)
   allocate(packed_accre_enhan(mgncol,nlev))
   packed_accre_enhan = xpack(packer,accre_enhan)

   allocate(packed_p(mgncol,nlev))
   packed_p = xpack(packer,state_loc%pmid)
   allocate(packed_pdel(mgncol,nlev))
   packed_pdel = xpack(packer,state_loc%pdel)

   allocate(packed_pint(mgncol,nlev+1))
   packed_pint = pack_interface(packer,state_loc%pint)

   allocate(packed_cldn(mgncol,nlev))
   packed_cldn = xpack(packer,ast)
   allocate(packed_liqcldf(mgncol,nlev))
   packed_liqcldf = xpack(packer,alst_mic)
   allocate(packed_icecldf(mgncol,nlev))
   packed_icecldf = xpack(packer,aist_mic)

   allocate(packed_naai(mgncol,nlev))
   packed_naai = xpack(packer,naai)
   allocate(packed_npccn(mgncol,nlev))
   packed_npccn = xpack(packer,npccn)

   allocate(packed_rndst(mgncol,nlev,size(rndst, 3)))
   packed_rndst = xpack(packer,rndst)
   allocate(packed_nacon(mgncol,nlev,size(nacon, 3)))
   packed_nacon = xpack(packer,nacon)

   if (.not. do_cldice) then
      allocate(packed_tnd_qsnow(mgncol,nlev))
      packed_tnd_qsnow = xpack(packer,tnd_qsnow)
      allocate(packed_tnd_nsnow(mgncol,nlev))
      packed_tnd_nsnow = xpack(packer,tnd_nsnow)
      allocate(packed_re_ice(mgncol,nlev))
      packed_re_ice = xpack(packer,re_ice)
   else
   ...
   if (use_hetfrz_classnuc) then
      ...
      packed_frzimm = xpack(packer,frzimm)
      allocate(packed_frzcnt(mgncol,nlev))
      packed_frzcnt = xpack(packer,frzcnt)
      allocate(packed_frzdep(mgncol,nlev))
      packed_frzdep = xpack(packer,frzdep)
   else
   ...
   do it = 1, num_steps
      ...
      packed_t = xpack(packer,state_loc%t)
      packed_q = xpack(packer,state_loc%q(:,:,1))
      packed_qc = xpack(packer,state_loc%q(:,:,ixcldliq))
      packed_nc = xpack(packer,state_loc%q(:,:,ixnumliq))
      packed_qi = xpack(packer,state_loc%q(:,:,ixcldice))
      packed_ni = xpack(packer,state_loc%q(:,:,ixnumice))
      if (micro_mg_version > 1) then
         packed_qr = xpack(packer,state_loc%q(:,:,ixrain))
         packed_nr = xpack(packer,state_loc%q(:,:,ixnumrain))
         packed_qs = xpack(packer,state_loc%q(:,:,ixsnow))
         packed_ns = xpack(packer,state_loc%q(:,:,ixnumsnow))
      end if

      call t_startf ( 'micro_mg_tend')
      select case (micro_mg_version)
      case (1)
         select case (micro_mg_sub_version)
         case (0)
      ...
      ! Set local tendency.
      ptend_loc%s               = xunpack(packer,packed_tlat, 0._r8)
      ptend_loc%q(:,:,1)        = xunpack(packer,packed_qvlat, 0._r8)
      ptend_loc%q(:,:,ixcldliq) = xunpack(packer,packed_qctend, 0._r8)
      ptend_loc%q(:,:,ixcldice) = xunpack(packer,packed_qitend, 0._r8)
      ptend_loc%q(:,:,ixnumliq) = xunpack(packer,packed_nctend, &
           -state_loc%q(:,:,ixnumliq)/(dtime/num_steps))
      if (do_cldice) then
         ptend_loc%q(:,:,ixnumice) = xunpack(packer,packed_nitend, &
      ...
      if (micro_mg_version > 1) then
         ptend_loc%q(:,:,ixrain)    = xunpack(packer,packed_qrtend, 0._r8)
         ptend_loc%q(:,:,ixsnow)    = xunpack(packer,packed_qstend, 0._r8)
         ptend_loc%q(:,:,ixnumrain) = xunpack(packer,packed_nrtend, &
              -state_loc%q(:,:,ixnumrain)/(dtime/num_steps))
         ptend_loc%q(:,:,ixnumsnow) = xunpack(packer,packed_nstend, &
   ...
   call process_and_unpack(post_proc)

   call finalize(post_proc)
   ...
   do k = top_lev, pver
	 ...
	 ! nanr - Microphysics fix for huge snow blobs (via M.Rothstein)  6jul16
