...
! call t_barrierf('sync_euler_step', hybrid%par%comm)
  ...
  if ( limiter_option == 8  ) then
    ...
    if ( rhs_multiplier == 2 ) then
      ...
      if ( nu_p > 0 ) then
        call t_startf('sw_euler_step_compute_rhs_2')
        ...
        call athread_join()
        call t_stopf('sw_euler_step_compute_rhs_2')
      endif
#ifdef OVERLAP
      ...
      call t_startf('sw_biharmonic_wk_scalar_euler_step')
      call biharmonic_wk_scalar(elem,qtens_biharmonic,deriv,edgeAdv,hybrid,nets,nete)
      call t_stopf('sw_biharmonic_wk_scalar_euler_step')
      call t_startf('sw_euler_step_compute_overlap2')
      ...
      call athread_join()
      call t_stopf('sw_euler_step_compute_overlap2')
...
!
    ...
    call t_stopf('sw_euler_step_rsh')
        ...
        do k = 1, nlev
          do j = 1, np
            do i = 1, np
			  ...
			  call print_binary(elem(ie)%state%Qdp(i,j,k,q,np1_qdp), elem(ie)%state%Qdp(i,j,k,q,n0_qdp))
            enddo
          enddo
        enddo
      enddo
...
#define VERTICAL_REMAP_SLAVE
#ifndef VERTICAL_REMAP_SLAVE 
  ...
  !type (element_t) :: elem_slave(nets:nete)
  !do ie = nets , nete
  !   elem_slave(ie)%state%dp3d(:,:,:,np1) = elem(ie)%state%dp3d(:,:,:,np1)
  !   elem_slave(ie)%state%v(:,:,1,:,np1) = elem(ie)%state%v(:,:,1,:,np1)
  !   elem_slave(ie)%state%v(:,:,2,:,np1) = elem(ie)%state%v(:,:,2,:,np1)
  !   elem_slave(ie)%state%t(:,:,:,np1) = elem(ie)%state%t(:,:,:,np1)
  !   elem_slave(ie)%state%Qdp(:,:,:,:,np1_qdp) = elem(ie)%state%Qdp(:,:,:,:,np1_qdp)
  !enddo
  ...
  call t_startf('vertical_remap_mpe')
...
!         call remap_velocityC(np1,dt,elem,fvm,hvcoord,ie)
  ...
  enddo
  call t_stopf('vertical_remap_mpe')
  
  !call t_startf('vertical_remap_slave')
  !call vertical_remap_mpe(hybrid,elem_slave,fvm,hvcoord,dt,np1,np1_qdp,nets,nete)
  !call t_stopf('vertical_remap_slave')
  ...
  use hybvcoord_mod, only : hvcoord_t
  use vertremap_mod, only : remap1, remap1_nofilter, remap_q_ppm ! _EXTERNAL (actually INTERNAL)
  use control_mod, only :  rsplit, tracer_transport_type, TRACER_GRIDTYPE_FVM , vert_remap_q_alg
  use parallel_mod, only : abortmp , par
  use hybrid_mod     , only : hybrid_t
  use derivative_mod, only : interpolate_gll2fvm_points
#if defined(_SPELT)
  use spelt_mod, only: spelt_struct
  ...
  type(spelt_struct), intent(inout) :: fvm(:)
  real (kind=real_kind) :: cdp(1:nep,1:nep,nlev,ntrac-1)
  real (kind=real_kind)  :: psc(nep,nep),
dpc(nep,nep,nlev),dpc_star(nep,nep,nlev)
#else
  type(fvm_struct), intent(inout) :: fvm(:)
  real (kind=real_kind) :: cdp(1:nc,1:nc,nlev,ntrac)
  real (kind=real_kind)  :: psc(nc,nc), dpc(nc,nc,nlev),dpc_star(nc,nc,nlev)
  ...
  !vertical_remap struct
  TYPE vertical_remap_param_t
    INTEGER(kind = 8) :: hyai_ptr , hybi_ptr
    INTEGER(kind = 8) :: dp3d_ptr , ps_v_ptr
    INTEGER(kind = 8) :: v_ptr , t_ptr , Qdp_ptr
    INTEGER(kind = 8) :: ie_address_diff
    REAL(kind = real_kind) :: ps0
    INTEGER :: net_len , vert_remap_q_alg
  END TYPE
  TYPE(vertical_remap_param_t) :: vertical_remap_param
  external :: slave_vertical_remap_parallel
#if USE_CUDA_FORTRAN
  call vertical_remap_cuda(elem,fvm,hvcoord,dt,np1,np1_qdp,nets,nete)
  return
#endif
  ...
  vertical_remap_param%hyai_ptr = loc(hvcoord%hyai(1))
  vertical_remap_param%hybi_ptr = loc(hvcoord%hybi(1))
  vertical_remap_param%dp3d_ptr = loc(elem(nets)%state%dp3d(1,1,1,np1))
  vertical_remap_param%ps_v_ptr = loc(elem(nets)%state%ps_v(1,1,np1))
  vertical_remap_param%v_ptr = loc(elem(nets)%state%v(1,1,1,1,np1))
  vertical_remap_param%t_ptr = loc(elem(nets)%state%t(1,1,1,np1))
  vertical_remap_param%Qdp_ptr = loc(elem(nets)%state%Qdp(1,1,1,1,np1_qdp))
  vertical_remap_param%ie_address_diff = loc(elem(nets+1)%state%dp3d(1,1,1,np1)) - loc(elem(nets)%state%dp3d(1,1,1,np1))
  vertical_remap_param%ps0 = hvcoord%ps0
  vertical_remap_param%net_len = nete-nets+1
  vertical_remap_param%vert_remap_q_alg = vert_remap_q_alg
   ! reference levels:
  !   dp(k) = (hyai(k+1)-hyai(k))*ps0 + (hybi(k+1)-hybi(k))*ps_v(i,j)
  !   hybi(1)=0          pure pressure at top of atmosphere
  !   hyai(1)=ptop
  ...
  !    (dp_star(k)-dp(k))/dt_q = (eta_dot_dpdn(i,j,k+1) - eta_dot_dpdn(i,j,k) )
  !
  call athread_spawn(slave_vertical_remap_parallel , vertical_remap_param)
  call athread_join()
