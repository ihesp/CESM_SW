...
use modal_aer_opt_mpe,only: modal_aero_sw_parallel, modal_aero_lw_parallel
...
subroutine aer_rad_props_sw(list_idx, state, pbuf,  nnite, idxnite, &
   ...
   if (nmodes > 0) then
#ifdef NO_RADAERO_CPE
      call modal_aero_sw(list_idx, state, pbuf, nnite, idxnite, &
                         tau, tau_w, tau_w_g, tau_w_f)
#else
      call modal_aero_sw_parallel(list_idx, state, pbuf, nnite, idxnite, &
                         tau, tau_w, tau_w_g, tau_w_f)
#endif
   else
      tau    (1:ncol,:,:) = 0._r8
...
#else
      call modal_aero_lw_parallel(list_idx, state, pbuf, odap_aer)
#endif
   else
      odap_aer = 0._r8
   end if
