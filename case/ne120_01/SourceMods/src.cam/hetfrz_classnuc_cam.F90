...
!      pmid  => state%pmid               )
   ...
   do i = 1, ncnst
      aer_cb(:state%ncol,:,i,state%lchnk) = aer_cb(:state%ncol,:,i,state%lchnk) * rho(:state%ncol,:)
      ...
      aer(:state%ncol,:,i,state%lchnk) = ptr2d(:state%ncol,:) * rho(:state%ncol,:)
   end do
   ...
   do i = 1, state%ncol
      do k = top_lev, pver
         call get_aer_num(i, k, ncnst, aer(:,:,:,state%lchnk), aer_cb(:,:,:,state%lchnk), rho(i,k), &
   ...
   call outfld('bc_num',        total_aer_num(:,:,1),    pcols, state%lchnk)
   call outfld('dst1_num',      total_aer_num(:,:,2),    pcols, state%lchnk)
   call outfld('dst3_num',      total_aer_num(:,:,3),    pcols, state%lchnk)

   call outfld('bcc_num',       coated_aer_num(:,:,1),   pcols, state%lchnk)
   call outfld('dst1c_num',     coated_aer_num(:,:,2),   pcols, state%lchnk)
   call outfld('dst3c_num',     coated_aer_num(:,:,3),   pcols, state%lchnk)

   call outfld('bcuc_num',      uncoated_aer_num(:,:,1), pcols, state%lchnk)
   call outfld('dst1uc_num',    uncoated_aer_num(:,:,2), pcols, state%lchnk)
   call outfld('dst3uc_num',    uncoated_aer_num(:,:,3), pcols, state%lchnk)

   call outfld('bc_a1_num',     total_interstitial_aer_num(:,:,1), pcols, state%lchnk)
   call outfld('dst_a1_num',    total_interstitial_aer_num(:,:,2), pcols, state%lchnk)
   call outfld('dst_a3_num',    total_interstitial_aer_num(:,:,3), pcols, state%lchnk)

   call outfld('bc_c1_num',     total_cloudborne_aer_num(:,:,1),   pcols, state%lchnk)
   call outfld('dst_c1_num',    total_cloudborne_aer_num(:,:,2),   pcols, state%lchnk)
   call outfld('dst_c3_num',    total_cloudborne_aer_num(:,:,3),   pcols, state%lchnk)

   call outfld('fn_bc_c1_num',  fn_cloudborne_aer_num(:,:,1),      pcols, state%lchnk)
   call outfld('fn_dst_c1_num', fn_cloudborne_aer_num(:,:,2),      pcols, state%lchnk)
   call outfld('fn_dst_c3_num', fn_cloudborne_aer_num(:,:,3),      pcols, state%lchnk)
        
   call outfld('na500',         na500,     pcols, state%lchnk)
   call outfld('totna500',      tot_na500, pcols, state%lchnk)
   ...
   frzimm(:state%ncol,:) = 0._r8
   frzcnt(:state%ncol,:) = 0._r8
   frzdep(:state%ncol,:) = 0._r8

   frzbcimm(:state%ncol,:) = 0._r8
   frzduimm(:state%ncol,:) = 0._r8
   frzbccnt(:state%ncol,:) = 0._r8
   frzducnt(:state%ncol,:) = 0._r8
   frzbcdep(:state%ncol,:) = 0._r8
   frzdudep(:state%ncol,:) = 0._r8

   freqimm(:state%ncol,:) = 0._r8
   freqcnt(:state%ncol,:) = 0._r8
   freqdep(:state%ncol,:) = 0._r8
   freqmix(:state%ncol,:) = 0._r8

   numice10s(:state%ncol,:)         = 0._r8
   numice10s_imm_dst(:state%ncol,:) = 0._r8
   numice10s_imm_bc(:state%ncol,:)  = 0._r8
   ...
   do i = 1, state%ncol
      do k = top_lev, pver
         ...
         if (state%t(i,k) > 235.15_r8 .and. state%t(i,k) < 269.15_r8) then
            ...
            ncic = max(state%q(i,k,numliq_idx)/lcldm(i,k), 0._r8)
   ...
   call outfld('FREQIMM', freqimm, pcols, state%lchnk)
   call outfld('FREQCNT', freqcnt, pcols, state%lchnk)
   call outfld('FREQDEP', freqdep, pcols, state%lchnk)
   call outfld('FREQMIX', freqmix, pcols, state%lchnk)

   call outfld('DSTFREZIMM', nnuccc_dst, pcols, state%lchnk)
   call outfld('DSTFREZCNT', nnucct_dst, pcols, state%lchnk)
   call outfld('DSTFREZDEP', nnudep_dst, pcols, state%lchnk)

   call outfld('BCFREZIMM', nnuccc_bc, pcols, state%lchnk)
   call outfld('BCFREZCNT', nnucct_bc, pcols, state%lchnk)
   call outfld('BCFREZDEP', nnudep_bc, pcols, state%lchnk)

   call outfld('NIMIX_IMM', niimm_bc+niimm_dst, pcols, state%lchnk)
   call outfld('NIMIX_CNT', nicnt_bc+nicnt_dst, pcols, state%lchnk)   
   call outfld('NIMIX_DEP', nidep_bc+nidep_dst, pcols, state%lchnk)

   call outfld('DSTNICNT', nicnt_dst, pcols, state%lchnk)
   call outfld('DSTNIDEP', nidep_dst, pcols, state%lchnk)
   call outfld('DSTNIIMM', niimm_dst, pcols, state%lchnk)

   call outfld('BCNICNT', nicnt_bc, pcols, state%lchnk)
   call outfld('BCNIDEP', nidep_bc, pcols, state%lchnk)
   call outfld('BCNIIMM', niimm_bc, pcols, state%lchnk)

   call outfld('NUMICE10s', numice10s, pcols, state%lchnk)
   call outfld('NUMIMM10sDST', numice10s_imm_dst, pcols, state%lchnk)
   call outfld('NUMIMM10sBC', numice10s_imm_bc, pcols, state%lchnk)
