...
!  4: concatenated blocks, no load balancing, no interprocess communication
   ...
   !================changed by guoheng==========================
   logical, private :: lbal_round = .false.  !whether round_robin is used
   !==============end change====================================
...
!========================================================================
  ...
  subroutine phys_grid_init( )
    ...
    !==========changed by guoheng=====================
    !write(*,*) "whether dynamics and physics need comm", local_dp_map
    !=====end change
    !
    ...
    !if(myid==0) then
    !    open(unit = 2345, file="chunkinit")
    !    write(2345, *) begchunk, endchunk
    !    call flush(2345)
    !    close(2345)
    !write(*,*) "chunkinit: myid,", myid, begchunk, endchunk
    !end if
    !===end change
    !
...
!-----------------------------------------------------------------------
     if ( present(phys_loadbalance_in) ) then
        ...
        !=======changed by guoheng=======
        !write(*,*) " in init function lbal_opt= ", lbal_opt
        !lbal_opt = 3 
        ! update phys_loadbalance
        if(lbal_opt .ge. 6) then
            lbal_opt = lbal_opt - 6
            lbal_round = .true.
        end if
        !==========end change==========
...
!
   call get_block_bounds_d(firstblock,lastblock)
   !===changed by guoheng
   !write(*,*) "block bounds", myid, firstblock, lastblock
...
!     assign processes in idle dynamics SMP nodes to virtual SMP nodes (wrap map)
      ...
      !if(myid == 0) then
          !output smp info
          !write(*,*) "nsmps = ", nsmps
          !output proc_smp_mapx
          !write(*,*) "proc_smp_mapx, p ==> smp"
          !do p=0, npes-1
          !  write(*,*) p, "==>", proc_smp_mapx(p)
        !end do
    !end if
    !======end change
...
!
   ...
   !=======changed by guoheng
   !write(*,*) "max_nproc_smpx", max_nproc_smpx
   ! max_nproc_smpx = 1 in 100P
   !=======end change
!
...
!
   ...
   !if(myid == 0) then
   !   do smp=0,nsmpx-1
   !     write(*,*) "nsmpcolumns", smp, nsmpcolumns(smp)
   !   end do
   ! end if
...
!
      !if(myid==0) then
      !    do smp=0,nsmpx-1
      !      write(*,*) "nsmpthreads ", nsmpthreads(smp)
      !      write(*,*) "nsmpcolumns ", nsmpcolumns(smp)
      !    end do
      !end if
      !write(*,*) "nsmpx is : ", nsmpx
      !write(*,*) "pcols is : ", pcols
      nchunks = 0
      do smp=0,nsmpx-1
...
!
      !=====changed by guoheng
      !do smp=1, nsmpx-1
      !  write(*,*) "smp, chunks", smp, nsmpchunks(smp)
      !end do
      !write(*,*) "firstblock, lastblock", firstblock, lastblock
      !====end change=====
...
!==========changed by guoheng=========================
   ...
   !                   nsmpthreads, nsmpchunks, round_robin=round_robin)
   call assign_chunks(npthreads, nsmpx, proc_smp_mapx, &
                      nsmpthreads, nsmpchunks, round_robin=lbal_round)
!============end change==============================
!
! Clean up
!
...
!========================================================================
   ...
   subroutine assign_chunks(npthreads, nsmpx, proc_smp_mapx, &
                            nsmpthreads, nsmpchunks, round_robin)
!----------------------------------------------------------------------- 
! 
...
!=====changed by guoheng=====
   logical, intent(in), optional    :: round_robin
!---------------------------Local workspace-----------------------------
...
!
   ...
   !===========changed by guoheng========
       round_smpid = 1 
       round_process = smp_proc_mapx(smp, round_smpid)
   !=======end change++++++++++++++++++
...
!  columns from chunk, from among those still available
         ...
         !========changed by guoheng===============
         if(random) then
             !calculate round_process
             do while(cur_npchunks(round_process) == npchunks(round_process))
                round_smpid = round_smpid + 1
                if(round_smpid .gt. ntsks_smpx(smp)) then
                    round_smpid = 1
                end if
                round_process = smp_proc_mapx(smp, round_smpid)
               ! round_process = round_process + 1
               ! if(round_process .ge. ntsks_smpx(smp)) then
               !     round_process = 0
               ! end if
            end do
             !assign to round_process
            ntmp2 = round_process
            ! update round_process
            round_smpid = round_smpid + 1
            if(round_smpid .gt. ntsks_smpx(smp)) then
                round_smpid = 1
            end if
