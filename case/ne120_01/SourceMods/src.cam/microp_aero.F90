...
subroutine microp_aero_run ( &
   ...
   ! initialize output
   npccn(1:state%ncol,1:pver)    = 0._r8  

   nacon(1:state%ncol,1:pver,:)  = 0._r8

   ! set default or fixed dust bins for contact freezing
   rndst(1:state%ncol,1:pver,1) = rn_dst1
   rndst(1:state%ncol,1:pver,2) = rn_dst2
   rndst(1:state%ncol,1:pver,3) = rn_dst3
   rndst(1:state%ncol,1:pver,4) = rn_dst4
   ...
   if (clim_modal_aero) then
      ...
      do m = 1, naer_all
         ...
         maerosol(:state%ncol,:,m) = aer_mmr(:state%ncol,:)*rho(:state%ncol,:)
         
         if (m .eq. idxsul) then
            naer2(:state%ncol,:,m) = maerosol(:state%ncol,:,m)*num_to_mass_aer(m)*bulk_scale
         else
            naer2(:state%ncol,:,m) = maerosol(:state%ncol,:,m)*num_to_mass_aer(m)
         end if
      end do
   end if
   ...
   case ('CLUBB_SGS')
      ...
      tke(:state%ncol,:) = (3._r8/2._r8)*wp2(:state%ncol,:)
   ...
   wsub(:state%ncol,:top_lev-1)  = 0.20_r8
   wsubi(:state%ncol,:top_lev-1) = 0.001_r8
   ...
   call outfld('WSUB',   wsub, pcols, state%lchnk)
   call outfld('WSUBI', wsubi, pcols, state%lchnk)
   ...
   if (clim_modal_aero) then
      ...
      do k = top_lev, pver
         do i = 1, state%ncol
            qcld = state%q(i,k,cldliq_idx) + state%q(i,k,cldice_idx)
            if (qcld > qsmall) then
               lcldn(i,k) = cldn(i,k)*state%q(i,k,cldliq_idx)/qcld
               lcldo(i,k) = cldo(i,k)*state%q(i,k,cldliq_idx)/qcld
            end if
         end do
      end do

      call outfld('LCLOUD', lcldn, pcols, state%lchnk)

      call dropmixnuc( &
         state, ptend, deltatin, pbuf, wsub, &
         lcldn, lcldo, nctend_mixnuc, factnum)

      npccn(:state%ncol,:) = nctend_mixnuc(:state%ncol,:)
      ...
      do k = top_lev, pver
         do i = 1, state%ncol
            ...
            if (state%q(i,k,cldliq_idx)>= qsmall) then

               ! get droplet activation rate

               call ndrop_bam_run( &
                  wsub(i,k), state%t(i,k), rho(i,k), naer2(i,k,:), naer_all, &
            ...
            npccn(i,k) = (dum*lcldm(i,k) - state%q(i,k,numliq_idx))/deltatin
         end do
      end do
   ...
   if (.not. clim_modal_aero) then
      ...
      call ndrop_bam_ccn(state%lchnk, state%ncol, maerosol, naer2)
