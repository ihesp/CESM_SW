...
! Taumol output
      ...
      real(kind=r8) :: ztaug(nlay, ngptsw)
      real(kind=r8) :: ztaur(nlay, ngptsw)
...
! Calculate the optical depths for gaseous absorption and Rayleigh scattering
         ...
         call spcvmc_sw &
             (lchnk, iplon, nlay, istart, iend, icpr, idelm, iout, &
              ...
              zsflxzen, ztaug, ztaur)
#else
         if ((icpr .lt. 1) .or. (idelm .eq. 0) .or. (iout .gt. 0)) then
            write(*,*) "icpr is", icpr, "idelm is", idelm, "iout is", iout
            stop "Can't use spcvmc_sw_parallel for unsupported icpr or idelm or iout(Can easily support)"
         endif
         call spcvmc_sw_parallel &
             (lchnk, iplon, nlay, istart, iend, icpr, idelm, iout, &
              pavel, tavel, pz, tz, tbound, albdif, albdir, &
              zcldfmc, ztaucmc, zasycmc, zomgcmc, ztaormc, &
              ztaua, zasya, zomga, cossza, coldry, wkl, adjflux, &	 
              ...
              zbbfddir, zbbcddir, zuvfddir, zuvcddir, znifddir, znicddir, zbbfsu, zbbfsd, &
              zsflxzen, ztaug, ztaur, &
              ngc, ngs, od_lo, tblint, bpade, exp_tbl)
#endif
