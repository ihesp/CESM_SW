  ...
  subroutine new_fieldlist_vdiff(ncnst,chen_new_vdiff)

    integer,              intent(in)  :: ncnst           ! Number of constituents
    type(vdiff_selector),intent(out) :: chen_new_vdiff

    allocate( chen_new_vdiff%fields( 3 + ncnst ) )
    chen_new_vdiff%fields = .false.

  end subroutine  new_fieldlist_vdiff
...
!    use time_manager,       only : is_first_step, get_nstep
    ...
    use coords_1d, only: Coords1D,chen_new_Coords1D,chen_Coords1D_section,&
           chen_Coords1D_finalize
    use linear_1d_operators, only : BoundaryType, BoundaryFixedLayer, &
         BoundaryData, BoundaryFlux,TriDiagDecomp,decomp_left_div,&
         decomp_finalize,boundary_cond_finalize,BoundaryCond
  ...
  !                concentration should be done very carefully in the future version.
    ...
    interface
       ...
       subroutine  vd_lu_qdecomp(decomp, &
    ...
    real(r8),allocatable, dimension(:) :: tmpp
    ...
    !p = chen_new_Coords1D(pint(:ncol,:))
    call chen_new_Coords1D(p,pint(:ncol,:))

    ! Boundary condition for a fixed concentration directly on a boundary
    ! interface (i.e. a boundary layer of size 0).
!    write(*,*) "compute_vdiff chenyuhu 0",__FILE__,__LINE__
    !interface_boundary = BoundaryFixedLayer(spread(0._r8, 1, ncol))
    allocate(tmpp(ncol))
   tmpp=spread(0._r8, 1,ncol)
    call BoundaryFixedLayer(tmpp,interface_boundary)
!    write(*,*) "compute_vdiff chenyuhu 1",__FILE__,__LINE__
  ...
  ! Modification : Why 'kvq' is not changed by molecular diffusion ? 
    ...
    if( do_molec_diff ) then
      ...
      !                cnst_fixed_ubc , mw_fac , ntop_molec 
        ...
        call chen_Coords1D_section(p,[1,ncol],[1,nbot_molec],p_molec)
    write(*,*) "chen_Coords  chenyuhu 0",__FILE__,__LINE__
        !molec_boundary = BoundaryFixedLayer(p%del(:,nbot_molec+1))
    !tmpp=p%del(:,nbot_molec+1)
    call BoundaryFixedLayer(p%del(:,nbot_molec+1),molec_boundary)
    !call BoundaryFixedLayer(tmpp,molec_boundary)
    write(*,*) "chen_Coords  chenyuhu 1",__FILE__,__LINE__
    ...
    if( diffuse(fieldlist,'u') .or. diffuse(fieldlist,'v') ) then
       ...
       call fin_vol_lu_decomp(decomp,ztodt, p, &
            coef_q=tau_damp_rate, coef_q_diff=kvm(:ncol,:)*dpidz_sq)
       

       call decomp_left_div(decomp,u(:ncol,:))
       call decomp_left_div(decomp,v(:ncol,:))
       call decomp_finalize(decomp)
    ...
    if( diffuse(fieldlist,'s') ) then
     ...
     ! Diffuse dry static energy
       ...
       if ( kvt_returned ) then 
          ...
          call BoundaryData(dse_top(:ncol),l_cond)

          !call decomp_left_div(decomp,dse(:ncol,:), &
          !     l_cond=BoundaryData(dse_top(:ncol)))
          call decomp_left_div(decomp,dse(:ncol,:), &
               l_cond)
          call boundary_cond_finalize(l_cond)
          ...
          call decomp_left_div(decomp,ttemp)
  ...
  !                for aerosol mass and aerosol number. 
    ...
    call fin_vol_lu_decomp(no_molec_decomp,ztodt, p, &
    ...
    do m = 1, ncnst
       ...
       if( diffuse(fieldlist,'q',m) ) then
           ...
           if( do_molec_diff .and. diffuse(fieldlistm,'q',m)) then
              ...
              call vd_lu_qdecomp( decomp,pcols , pver   , ncol              , cnst_fixed_ubc(m), cnst_mw(m), &
              ...
              if ((cnst_fixed_ubflx(m))) then
                 ...
                 call decomp_left_div(decomp,q(:ncol,:,m), &
                      l_cond=BoundaryFlux( &
                      spread(-gravit*ubc_flux(m),1,ncol), ztodt, &
                      p%del(:,1)))

              else
                 call BoundaryData(ubc_mmr(:ncol,m),l_cond)
              ...
              call decomp_left_div(no_molec_decomp,q(:ncol,:,m))
    ...
    call decomp_finalize(no_molec_decomp)
    !call p%finalize()
