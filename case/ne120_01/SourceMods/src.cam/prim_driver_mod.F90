...
contains
  ...
  subroutine prim_init1(elem, fvm, par, Tl)
    ...
    use static_part_mod, only : genstaticpart
    ! --------------------------------
    use dof_mod, only : global_dof, CreateUniqueIndex, SetElemOffset
    ! --------------------------------
    use params_mod, only : SFCURVE, STATICPART
    ! --------------------------------
    ...
    if(partmethod .eq. STATICPART) then
       if(par%masterproc) write(iulog, *) "partitioning graph using static assgienment in static-assignment.txt"
       call genstaticpart(GridEdge, GridVertex, par)
    else if(partmethod .eq. SFCURVE) then
       if (par%masterproc) print *, "SFCURVE"
