...
module namelist_mod
  ...
  !-----------------
  use params_mod, only : recursive, sfcurve, staticpart
  !-----------------
  ...
  !-----------------
  use thread_mod, only : omp_get_max_threads, max_num_threads, horz_num_threads, vert_num_threads
  !-----------------
...
#else
    namelist /ctl_nl/ qsize &! number of SE tracers
...
!   write(iulog,*) "masterproc=",par%masterproc
    ...
    if (par%masterproc) then
      ...
      !=================================
      if (PARTMETHOD /= SFCURVE .and. PARTMETHOD /= STATICPART) then
         PARTMETHOD = SFCURVE
      end if
#endif
       ! ========================
