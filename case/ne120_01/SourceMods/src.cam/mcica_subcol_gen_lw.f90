...
!
      ...
      do i = 1, size(ran_arr)
         kiss = 69069_i8 * seed1(i) + 1327217885
          if (big_endian) then
             seed1(i) = ishft(kiss,bit_size(1))
          else
             seed1(i) = kiss
          end if
        seed2(i) = ieor( ieor(ieor (seed2(i), ishft (seed2(i), 13) ),ishft(ieor (seed2(i), ishft (seed2(i), 13) ),-17)),ishft(ieor(ieor (seed2(i), ishft (seed2(i), 13) ),ishft(ieor (seed2(i), ishft (seed2(i), 13) ),-17)),5))
         seed3(i) = 18000 * iand (seed3(i), 65535) + ishft (seed3(i), - 16)
         seed4(i) = 30903 * iand (seed4(i), 65535) + ishft (seed4(i), - 16)
         kiss = int(seed1(i), 8) + seed2(i) + ishft (seed3(i), 16) + seed4(i)
          if (big_endian) then
             kiss_i4 = ishft(kiss,bit_size(1))
             ran_arr(i) = kiss_i4*2.328306e-10_r8 + 0.5_r8
          else
             kiss_i4 = kiss
             ran_arr(i) = kiss_i4*2.328306e-10_r8 + 0.5_r8
          end if
