...
function vd_lu_decomp(dt, dp, coef_q,  coef_q_d, coef_q_d2, upper_bndry, &
  ...
  use linear_1d_operators, only: &
       ...
       second_derivative, &
       BoundaryType,      &
       lmult_as_diag,chen_add,&
       chen_new_TriDiagDecomp,&
       chen_tridiag_finalize
  ...
  type(BoundaryType), target, intent(in), optional :: &
  ...
  if (present(coef_q)) then
     ...
     call diagonal_operator(1._r8 - dt*coef_q,net_operator)
  else
     !net_operator = identity_operator(size(dp, 1), size(dp, 2) + 1)
     call identity_operator(size(dp, 1), size(dp, 2) + 1,net_operator)
  end if

  if (present(coef_q_d)) then
     !add_term = first_derivative(dp, add_term,upper_bndry, lower_bndry)
     call first_derivative(dp, add_term,upper_bndry, lower_bndry)
     !call add_term%lmult_as_diag(-dt*coef_q_d)
     call lmult_as_diag(add_term,-dt*coef_q_d)
     !call net_operator%add(add_term)
     call chen_add(net_operator,add_term)
  end if

  if (present(coef_q_d2)) then
     !add_term = second_derivative(dp, upper_bndry, lower_bndry)
     call second_derivative(dp,add_term, upper_bndry, lower_bndry)
     !call add_term%lmult_as_diag(-dt*coef_q_d2)
     call lmult_as_diag(add_term,-dt*coef_q_d2)
     !call net_operator%add(add_term)
     call chen_add(net_operator,add_term)
  end if

  !decomp = TriDiagDecomp(net_operator)
  call chen_new_TriDiagDecomp(net_operator,decomp)

  call chen_tridiag_finalize(net_operator)
  call chen_tridiag_finalize(add_term)
...
subroutine fin_vol_lu_decomp(decomp,dt, p, coef_q, coef_q_diff, coef_q_adv, &
     coef_q_weight, upper_bndry, lower_bndry, graft_decomp)
  ...
  use linear_1d_operators, only: &
       ...
       diffusion_operator, &
       advection_operator, &
       BoundaryType,lmult_as_diag,&
       chen_add,chen_new_TriDiagDecomp,&
       chen_tridiag_finalize,add_to_diag
  ...
  if (present(coef_q_diff)) then
     ...
     !     upper_bndry, lower_bndry)
     call diffusion_operator(p, coef_q_diff, &
          net_operator,upper_bndry, lower_bndry)
  else
     !net_operator = zero_operator(p%n, p%d)
     call zero_operator(p%n, p%d,net_operator)
  end if

  ! Constant term (damping).
  if (present(coef_q)) then
     !add_term = diagonal_operator(coef_q)
     call diagonal_operator(coef_q,add_term)
     !call net_operator%add(add_term)
     call chen_add(net_operator,add_term)
  end if
  ...
  if (present(coef_q_adv)) then
     ...
     !     upper_bndry, lower_bndry)
     call advection_operator(p, coef_q_adv,add_term, &
          upper_bndry, lower_bndry)
     !call net_operator%add(add_term)
     call chen_add(net_operator,add_term)
  end if
  ...
  if (present(coef_q_weight)) then
     ...
     call lmult_as_diag(net_operator,-dt/coef_q_weight)
  else
     !call net_operator%lmult_as_diag(-dt)
     call lmult_as_diag(net_operator,-dt)
  end if
  !call net_operator%add_to_diag(1._r8)
  call add_to_diag(net_operator,1._r8)
  ...
  call chen_new_TriDiagDecomp(net_operator,decomp, graft_decomp=graft_decomp)
...
end subroutine fin_vol_lu_decomp
