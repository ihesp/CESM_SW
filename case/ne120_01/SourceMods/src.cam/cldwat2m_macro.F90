  ...
  !--------------------------------------------------- !
   ...
   use cldfrc2m,         only: astG_PDF_single, astG_PDF, astG_RHU_single, &
    ...
    !use wv_sat_methods, only: tmp1, tmp2, tmpind, wvflag
   implicit none
   private
   save
   ...
   subroutine diffres2d(in1, in2, d1, d2, l1, l2, sname)
    integer, intent(in) :: d1, d2, l1, l2
    real(8), intent(in) :: in1(d2, d1)
    real(8), intent(in) :: in2(d1, d2)
    ...
    integer mid
include 'mpif.h'
    call MPI_comm_rank(MPI_COMM_WORLD, mid, ierr)

    mpi_id = mid
    !if(mid == 31 .or. mid == 34 .or. mid == 35) then
    !if(masterproc) then
        do i = 1, l1
            do k = 1, l2
                if( isnan(in1(k,i)) ) then
                    write(*,*) sname,"NAN", i, k, in1(k,i), in2(i,k)
                endif
                if ( in2(i,k) .ne. 0.d0) then
                if(abs((in1(k,i) - in2(i,k)) / in2(i,k)) > 1.d-10) then
                    write(*,*) sname, "1st", mid, i, k, abs((in1(k,i) - in2(i,k)) / in2(i,k)), in1(k,i), in2(i,k)
                endif
            else
                if( abs((in1(k,i) - in2(i,k))) .ne. 0.d0) then
                    write(*,*) sname,"2nd",  mid, i, k, abs((in1(k,i) - in2(i,k))), in1(k,i), in2(i,k)
                endif
            endif
            enddo
        enddo
   ...
   subroutine diffres2dpre(in1, in2, d1, d2, l1, l2, small, sname)
    integer, intent(in) :: d1, d2, l1, l2
    real(8), intent(in) :: in1(d2, d1)
    real(8), intent(in) :: in2(d1, d2)
    ...
    if(masterproc) then
        do i = 1, l1
            do k = 1, l2
                ...
                if(abs((in1(k,i) - in2(i,k)) / in2(i,k)) > small) then
                    write(*,*) sname, i, k, abs((in1(k,i) - in2(i,k)) / in2(i,k)), in1(k,i), in2(i,k)
                endif
            enddo
        enddo
    endif
   ...
   subroutine diffres0d(in1, in2, i, k, sname)
    ...
    if(masterproc) then
        !do i = 1, l1
            !do k = 1, l2
                if( isnan(in1) ) then
                    write(*,*) sname,"NAN", i, k, in1, in2
                endif
                if(abs((in1 - in2) / in2) > 1.d-12) then
                    write(*,*) sname, i, k, abs((in1 - in2) / in2), in1, in2
                endif
            !enddo
        !enddo
    endif
   ...
   subroutine diffres3d(in1, in2, in3, d1, d2, l1, l2, sname)
    integer, intent(in) :: d1, d2, l1, l2
    real(8), intent(in) :: in1(d2, d1)
    real(8), intent(in) :: in2(d1, d2)
    real(8), intent(in) :: in3(d2, d1)
    character(len=*), intent(in) :: sname
    integer :: i, k
    if(masterproc) then
        do i = 1, l1
            do k = 1, l2
                ...
                if(abs((in1(k,i) - in2(i,k)) / in2(i,k)) > 1.d-16) then
                    !write(*,*) sname, i, k,  in3(k,i), tmp1(i,k)
                    write(*,*) sname, i, k, abs((in1(k,i) - in2(i,k)) / in2(i,k)), in1(k,i), in2(i,k)
                endif
            enddo
        enddo
   ...
   subroutine diffres2dbe(in1, in2, d1, d2, l0, l1, l2, l3, sname)
    integer, intent(in) :: d1, d2, l0, l1, l2, l3
    real(8), intent(in) :: in1(d2, d1)
    real(8), intent(in) :: in2(d1, d2)
    character(len=*), intent(in) :: sname
    integer :: i, k
   ...
   subroutine diffres2d6i(in1, in2, in3, in4, in5, in6, d1, d2, l0, l1, l2, l3, sname)
    integer, intent(in) :: d1, d2, l0, l1, l2, l3
    real(8), intent(in) :: in1(d2, d1)
    real(8), intent(in) :: in2(d1, d2)
    real(8), intent(in) :: in3(d2, d1)
    real(8), intent(in) :: in4(d2, d1)
    real(8), intent(in) :: in5(d1, d2)
    real(8), intent(in) :: in6(d1, d2)
    character(len=*), intent(in) :: sname
    integer :: i, k
    if(masterproc) then
        do i = l0, l1
            do k = l2, l3
                ...
                if(abs((in1(k,i) - in2(i,k)) / in2(i,k)) > 1.d-16) then
                    write(*,*) "n0tend", i, k, abs((in3(k,i) - in5(i,k)) / in5(i,k)), in3(k,i), in5(i,k)
                    write(*,*) "n1tend", i, k, abs((in4(k,i) - in6(i,k)) / in6(i,k)), in4(k,i), in6(i,k)
                    write(*,*) sname, i, k, abs((in1(k,i) - in2(i,k)) / in2(i,k)), in1(k,i), in2(i,k)
                endif
            enddo
        enddo
   ...
   subroutine diffres1d(in1, in2, d1, d2, l1, sname)
    integer, intent(in) :: d1, d2, l1
    real(8), intent(in) :: in1(d2, d1)
    real(8), intent(in) :: in2(d1)
    character(len=*), intent(in) :: sname
    integer :: i, k
    !if(masterproc) then
    k = 1
        do i = 1, l1
                if(abs((in1(k,i) - in2(i)) / in2(i)) > 1.d-10) then
                    write(*,*) sname, i,  abs((in1(k,i) - in2(i)) / in2(i)), in1(k,i), in2(i)
                endif
        enddo
    !endif
   end subroutine
   ...
   use wv_saturation,    only : findsp_vc
    use physconst,    only: epsilo, &
                            ...
                            h2otrip
    use perf_mod
    use cldfrc2m,         only: icecrit_cpe, iceopt_cpe 
   integer   icol
...
!xukai+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   real(r8) :: T0_trans(pver,pcols)               ! Temperature [K]
   real(r8) :: qv0_trans(pver,pcols)              ! Grid-mean water vapor specific humidity [kg/kg]
   real(r8) :: ql0_trans(pver,pcols)              ! Grid-mean liquid water content [kg/kg]
   real(r8) :: qi0_trans(pver,pcols)              ! Grid-mean ice water content [kg/kg]
   real(r8) :: nl0_trans(pver,pcols)              ! Grid-mean number concentration of cloud liquid droplet [#/kg]
   real(r8) :: ni0_trans(pver,pcols)              ! Grid-mean number concentration of cloud ice    droplet [#/kg]

   ! Input variables

   real(r8)    :: p_trans(pver,pcols)                ! Pressure at the layer mid-point [Pa]
   real(r8)    :: dp_trans(pver,pcols)               ! Pressure thickness [Pa] > 0

   real(r8)    :: A_T_trans(pver,pcols)              ! Non-microphysical advective external forcing of T  [K/s]
   real(r8)    :: A_qv_trans(pver,pcols)             ! Non-microphysical advective external forcing of qv [kg/kg/s]
   real(r8)    :: A_ql_trans(pver,pcols)             ! Non-microphysical advective external forcing of ql [kg/kg/s]
   real(r8)    :: A_qi_trans(pver,pcols)             ! Non-microphysical advective external forcing of qi [kg/kg/s]
   real(r8)    :: A_nl_trans(pver,pcols)             ! Non-microphysical advective external forcing of nl [#/kg/s]
   real(r8)    :: A_ni_trans(pver,pcols)             ! Non-microphysical advective external forcing of ni [#/kg/s] 

   real(r8)    :: C_T_trans(pver,pcols)              ! Microphysical advective external forcing of T  [K/s]
   real(r8)    :: C_qv_trans(pver,pcols)             ! Microphysical advective external forcing of qv [kg/kg/s]
   real(r8)    :: C_ql_trans(pver,pcols)             ! Microphysical advective external forcing of ql [kg/kg/s]
   real(r8)    :: C_qi_trans(pver,pcols)             ! Microphysical advective external forcing of qi [kg/kg/s]
   real(r8)    :: C_nl_trans(pver,pcols)             ! Microphysical advective external forcing of nl [#/kg/s]
   real(r8)    :: C_ni_trans(pver,pcols)             ! Microphysical advective external forcing of ni [#/kg/s] 
   real(r8)    :: C_qlst_trans(pver,pcols)           ! Microphysical advective external forcing of ql
                                                           ! within liquid stratus [kg/kg/s]

   real(r8)    :: D_T_trans(pver,pcols)              ! Cumulus detrainment external forcing of T  [K/s]
   real(r8)    :: D_qv_trans(pver,pcols)             ! Cumulus detrainment external forcing of qv [kg/kg/s]
   real(r8)    :: D_ql_trans(pver,pcols)             ! Cumulus detrainment external forcing of ql [kg/kg/s]
   real(r8)    :: D_qi_trans(pver,pcols)             ! Cumulus detrainment external forcing of qi [kg/kg/s]
   real(r8)    :: D_nl_trans(pver,pcols)             ! Cumulus detrainment external forcing of nl [#/kg/s]
   real(r8)    :: D_ni_trans(pver,pcols)             ! Cumulus detrainment external forcing of qi [#/kg/s] 

   real(r8)    :: a_cud_trans(pver,pcols)            ! Old cumulus fraction before update
   real(r8)    :: a_cu0_trans(pver,pcols)            ! New cumulus fraction after update

   real(r8)    :: clrw_old_trans(pver,pcols)         ! Clear sky fraction at the previous time step for liquid stratus process
   real(r8)    :: clri_old_trans(pver,pcols)         ! Clear sky fraction at the previous time step for    ice stratus process
   real(r8)    :: tke_trans(pverp,pcols)                     ! (pcols,pverp) TKE from the PBL scheme
   real(r8)    :: qtl_flx_trans(pverp,pcols)                 ! (pcols,pverp) overbar(w'qtl') from PBL scheme where qtl = qv + ql
   real(r8)    :: qti_flx_trans(pverp,pcols)                 ! (pcols,pverp) overbar(w'qti') from PBL scheme where qti = qv + qi
   real(r8)    :: cmfr_det_trans(pver,pcols)                ! (pcols,pver)  Detrained mass flux from the convection scheme
   real(r8)    :: qlr_det_trans(pver,pcols)                 ! (pcols,pver)  Detrained        ql from the convection scheme
   real(r8)    :: qir_det_trans(pver,pcols)                 ! (pcols,pver)  Detrained        qi from the convection scheme


   ! Output variables

   real(r8)   :: s_tendout_trans(pver,pcols)        ! Net tendency of grid-mean s  from 'Micro+Macro' processes [J/kg/s]
   real(r8)   :: qv_tendout_trans(pver,pcols)       ! Net tendency of grid-mean qv from 'Micro+Macro' processes [kg/kg/s]
   real(r8)   :: ql_tendout_trans(pver,pcols)       ! Net tendency of grid-mean ql from 'Micro+Macro' processes [kg/kg/s]
   real(r8)   :: qi_tendout_trans(pver,pcols)       ! Net tendency of grid-mean qi from 'Micro+Macro' processes [kg/kg/s]
   real(r8)   :: nl_tendout_trans(pver,pcols)       ! Net tendency of grid-mean nl from 'Micro+Macro' processes [#/kg/s]
   real(r8)   :: ni_tendout_trans(pver,pcols)       ! Net tendency of grid-mean ni from 'Micro+Macro' processes [#/kg/s]

   real(r8)   :: qme_trans(pver,pcols)              ! Net condensation rate [kg/kg/s]
   real(r8)   :: qvadj_trans(pver,pcols)            ! adjustment tendency from "positive_moisture" call (vapor)
   real(r8)   :: qladj_trans(pver,pcols)            ! adjustment tendency from "positive_moisture" call (liquid)
   real(r8)   :: qiadj_trans(pver,pcols)            ! adjustment tendency from "positive_moisture" call (ice)
   real(r8)   :: qllim_trans(pver,pcols)            ! tendency from "instratus_condensate" call (liquid)
   real(r8)   :: qilim_trans(pver,pcols)            ! tendency from "instratus_condensate" call (ice)

   real(r8)   :: cld_trans(pver,pcols)              ! Net cloud fraction ( 0 <= cld <= 1 )
   real(r8)   :: al_st_star_trans(pver,pcols)       ! Physical liquid stratus fraction
   real(r8)   :: ai_st_star_trans(pver,pcols)       ! Physical ice stratus fraction
   real(r8)   :: ql_st_star_trans(pver,pcols)       ! In-stratus LWC [kg/kg] 
   real(r8)   :: qi_st_star_trans(pver,pcols)       ! In-stratus IWC [kg/kg] 

   real(r8)   :: d_rhmin_liq_PBL_trans(pver,pcols)
   real(r8)   :: d_rhmin_ice_PBL_trans(pver,pcols)
   real(r8)   :: d_rhmin_liq_det_trans(pver,pcols)
   real(r8)   :: d_rhmin_ice_det_trans(pver,pcols)
 

    type mmacro_pcond_args  
   ! two dimensions

   ! Input-Output variables

   integer(8) :: T0               ! Temperature [K]
   integer(8) :: qv0              ! Grid-mean water vapor specific humidity [kg/kg]
   integer(8) :: ql0              ! Grid-mean liquid water content [kg/kg]
   integer(8) :: qi0              ! Grid-mean ice water content [kg/kg]
   integer(8) :: nl0              ! Grid-mean number concentration of cloud liquid droplet [#/kg]
   integer(8) :: ni0              ! Grid-mean number concentration of cloud ice    droplet [#/kg]

   ! Input variables

   integer(8)    :: p                ! Pressure at the layer mid-point [Pa]
   integer(8)    :: dp               ! Pressure thickness [Pa] > 0

   integer(8)    :: A_T              ! Non-microphysical advective external forcing of T  [K/s]
   integer(8)    :: A_qv             ! Non-microphysical advective external forcing of qv [kg/kg/s]
   integer(8)    :: A_ql             ! Non-microphysical advective external forcing of ql [kg/kg/s]
   integer(8)    :: A_qi             ! Non-microphysical advective external forcing of qi [kg/kg/s]
   integer(8)    :: A_nl             ! Non-microphysical advective external forcing of nl [#/kg/s]
   integer(8)    :: A_ni             ! Non-microphysical advective external forcing of ni [#/kg/s] 

   integer(8)    :: C_T              ! Microphysical advective external forcing of T  [K/s]
   integer(8)    :: C_qv             ! Microphysical advective external forcing of qv [kg/kg/s]
   integer(8)    :: C_ql             ! Microphysical advective external forcing of ql [kg/kg/s]
   integer(8)    :: C_qi             ! Microphysical advective external forcing of qi [kg/kg/s]
   integer(8)    :: C_nl             ! Microphysical advective external forcing of nl [#/kg/s]
   integer(8)    :: C_ni             ! Microphysical advective external forcing of ni [#/kg/s] 
   integer(8)    :: C_qlst           ! Microphysical advective external forcing of ql
                                                           ! within liquid stratus [kg/kg/s]

   integer(8)    :: D_T              ! Cumulus detrainment external forcing of T  [K/s]
   integer(8)    :: D_qv             ! Cumulus detrainment external forcing of qv [kg/kg/s]
   integer(8)    :: D_ql             ! Cumulus detrainment external forcing of ql [kg/kg/s]
   integer(8)    :: D_qi             ! Cumulus detrainment external forcing of qi [kg/kg/s]
   integer(8)    :: D_nl             ! Cumulus detrainment external forcing of nl [#/kg/s]
   integer(8)    :: D_ni             ! Cumulus detrainment external forcing of qi [#/kg/s] 

   integer(8)    :: a_cud            ! Old cumulus fraction before update
   integer(8)    :: a_cu0            ! New cumulus fraction after update

   integer(8)    :: clrw_old         ! Clear sky fraction at the previous time step for liquid stratus process
   integer(8)    :: clri_old         ! Clear sky fraction at the previous time step for    ice stratus process

   integer(8)       :: tke                     ! (pcols,pverp) TKE from the PBL scheme
   integer(8)       :: qtl_flx                 ! (pcols,pverp) overbar(w'qtl') from PBL scheme where qtl = qv + ql
   integer(8)       :: qti_flx                 ! (pcols,pverp) overbar(w'qti') from PBL scheme where qti = qv + qi
   integer(8)       :: cmfr_det                ! (pcols,pver)  Detrained mass flux from the convection scheme
   integer(8)       :: qlr_det                 ! (pcols,pver)  Detrained        ql from the convection scheme
   integer(8)       :: qir_det                 ! (pcols,pver)  Detrained        qi from the convection scheme
   ! Output variables

   integer(8)   :: s_tendout        ! Net tendency of grid-mean s  from 'Micro+Macro' processes [J/kg/s]
   integer(8)   :: qv_tendout       ! Net tendency of grid-mean qv from 'Micro+Macro' processes [kg/kg/s]
   integer(8)   :: ql_tendout       ! Net tendency of grid-mean ql from 'Micro+Macro' processes [kg/kg/s]
   integer(8)   :: qi_tendout       ! Net tendency of grid-mean qi from 'Micro+Macro' processes [kg/kg/s]
   integer(8)   :: nl_tendout       ! Net tendency of grid-mean nl from 'Micro+Macro' processes [#/kg/s]
   integer(8)   :: ni_tendout       ! Net tendency of grid-mean ni from 'Micro+Macro' processes [#/kg/s]

   integer(8)   :: qme              ! Net condensation rate [kg/kg/s]
   integer(8)   :: qvadj            ! adjustment tendency from "positive_moisture" call (vapor)
   integer(8)   :: qladj            ! adjustment tendency from "positive_moisture" call (liquid)
   integer(8)   :: qiadj            ! adjustment tendency from "positive_moisture" call (ice)
   integer(8)   :: qllim            ! tendency from "instratus_condensate" call (liquid)
   integer(8)   :: qilim            ! tendency from "instratus_condensate" call (ice)

   integer(8)   :: cld              ! Net cloud fraction ( 0 <= cld <= 1 )
   integer(8)   :: al_st_star       ! Physical liquid stratus fraction
   integer(8)   :: ai_st_star       ! Physical ice stratus fraction
   integer(8)   :: ql_st_star       ! In-stratus LWC [kg/kg] 
   integer(8)   :: qi_st_star       ! In-stratus IWC [kg/kg] 

   integer(8)   :: d_rhmin_liq_PBL
   integer(8)   :: d_rhmin_ice_PBL
   integer(8)   :: d_rhmin_liq_det
   integer(8)   :: d_rhmin_ice_det
 


   !one dimension 
   ! Input variables 
   integer(8)    :: landfrac              ! Land fraction
   integer(8)    :: snowh                 ! Snow depth (liquid water equivalent)
   integer(8)    :: wv_para
   integer(8)    :: endrunflag

   real(8)      :: dt                           ! Model integration time step [s]

   real(8) :: rhmini_const
   real(8) :: rhmaxi_const
   real(8) :: rhminl_const
   real(8) :: rhminl_adj_land_const
   real(8) :: rhminh_const
   real(8) :: qmin1
   real(8) :: qmin2
   real(8) :: qmin3
   ...
   integer :: top_lev
   integer :: ncol
   integer :: do_cldice_int 
   integer :: i_rhmini
   integer :: i_rhminl 
   integer :: iceopt 

    end type
    type(mmacro_pcond_args) :: mmacro_pcond_para
    integer, external ::  slave_mmacro_pcond_cpe_c
    integer, external :: slave_setftz_f
    integer, external :: slave_unsetftz_f 
    real(r8) :: wv_para(16)
    integer :: endrunflag(128)
    integer, target :: tmppind(pcols)
    real(r8), target :: tmpp1(pcols, pver)
    real(r8), target :: tmpp2(pcols, pver)
    real(r8), target :: tmpp3(pcols, pver)
    real(r8), target :: tmpp4(pcols, pver)
    real(8) :: qminslave


!xukai----------------------------------------------------------------------------------------------------------------------------------------------------------------
!xukai++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   call t_startf("macro_cldwa_trans") 
    !tmp1 => tmpp1(:,:)
    !tmp2 => tmpp2(:,:)
    !tmpind => tmppind(:)
    do i = 1, pcols
   ...
   ! Input-Output variables
    ...
    T0_trans(k,i)        =  T0(i,k)             
    qv0_trans(k,i)       =  qv0(i,k)            
    ql0_trans(k,i)       =  ql0(i,k)            
    qi0_trans(k,i)       =  qi0(i,k)            
    nl0_trans(k,i)       =  nl0(i,k)            
    ni0_trans(k,i)       =  ni0(i,k)            
   ...
   ! Input variables                Input variables
    ...
    A_T_trans(k,i)       =  A_T(i,k)            
    A_qv_trans(k,i)      =  A_qv(i,k)           
    A_ql_trans(k,i)      =  A_ql(i,k)           
    A_qi_trans(k,i)      =  A_qi(i,k)           
    A_nl_trans(k,i)      =  A_nl(i,k)           
    A_ni_trans(k,i)      =  A_ni(i,k)           
                                                              
    C_T_trans(k,i)       =  C_T(i,k)            
    C_qv_trans(k,i)      =  C_qv(i,k)           
    C_ql_trans(k,i)      =  C_ql(i,k)           
    C_qi_trans(k,i)      =  C_qi(i,k)           
    C_nl_trans(k,i)      =  C_nl(i,k)           
    C_ni_trans(k,i)      =  C_ni(i,k)           
    C_qlst_trans(k,i)    =  C_qlst(i,k)         
                                                              
    D_T_trans(k,i)       =  D_T(i,k)            
    D_qv_trans(k,i)      =  D_qv(i,k)           
    D_ql_trans(k,i)      =  D_ql(i,k)           
    D_qi_trans(k,i)      =  D_qi(i,k)           
    D_nl_trans(k,i)      =  D_nl(i,k)           
    D_ni_trans(k,i)      =  D_ni(i,k)           
                                                              
    a_cud_trans(k,i)     =  a_cud(i,k)          
    a_cu0_trans(k,i)     =  a_cu0(i,k)          
                                                              
    clrw_old_trans(k,i)  =  clrw_old(i,k)       
    clri_old_trans(k,i)  =  clri_old(i,k)       

    !!pointer
    !tke_trans(k,i)       =  tke(i,k)                   
    !qtl_flx_trans(k,i)   =  qtl_flx(i,k)               
    !qti_flx_trans(k,i)   =  qti_flx(i,k)               
    !cmfr_det_trans(k,i)  =  cmfr_det(i,k)              
    !qlr_det_trans(k,i)   =  qlr_det(i,k)               
    !qir_det_trans(k,i)   =  qir_det(i,k)               
    enddo
    enddo
    ...
    !tke_trans(pverp,pcols)      =  tke(pverp,pcols)                   
    !qtl_flx_trans(pverp,pcols)  =  qtl_flx(pverp,pcols)               
    !qti_flx_trans(pverp,pcols)  =  qti_flx(pverp,pcols)               

   ! Input-Output variables

   mmacro_pcond_para%T0           = loc(T0_trans(:,:))      
   mmacro_pcond_para%qv0          = loc(qv0_trans(:,:))     
   mmacro_pcond_para%ql0          = loc(ql0_trans(:,:))     
   mmacro_pcond_para%qi0          = loc(qi0_trans(:,:))     
   mmacro_pcond_para%nl0          = loc(nl0_trans(:,:))     
   mmacro_pcond_para%ni0          = loc(ni0_trans(:,:))     

   ! Input variables

   mmacro_pcond_para%p            = loc(p_trans(:,:))     
   mmacro_pcond_para%dp           = loc(dp_trans(:,:))     

   mmacro_pcond_para%A_T          = loc(A_T_trans(:,:))     
   mmacro_pcond_para%A_qv         = loc(A_qv_trans(:,:))     
   mmacro_pcond_para%A_ql         = loc(A_ql_trans(:,:))     
   mmacro_pcond_para%A_qi         = loc(A_qi_trans(:,:))     
   mmacro_pcond_para%A_nl         = loc(A_nl_trans(:,:))     
   mmacro_pcond_para%A_ni         = loc(A_ni_trans(:,:))     

   mmacro_pcond_para%C_T          = loc(C_T_trans(:,:))     
   mmacro_pcond_para%C_qv         = loc(C_qv_trans(:,:))     
   mmacro_pcond_para%C_ql         = loc(C_ql_trans(:,:))     
   mmacro_pcond_para%C_qi         = loc(C_qi_trans(:,:))     
   mmacro_pcond_para%C_nl         = loc(C_nl_trans(:,:))     
   mmacro_pcond_para%C_ni         = loc(C_ni_trans(:,:))     
   mmacro_pcond_para%C_qlst       = loc(C_qlst_trans(:,:))     

   mmacro_pcond_para%D_T          = loc(D_T_trans(:,:))     
   mmacro_pcond_para%D_qv         = loc(D_qv_trans(:,:))     
   mmacro_pcond_para%D_ql         = loc(D_ql_trans(:,:))     
   mmacro_pcond_para%D_qi         = loc(D_qi_trans(:,:))     
   mmacro_pcond_para%D_nl         = loc(D_nl_trans(:,:))     
   mmacro_pcond_para%D_ni         = loc(D_ni_trans(:,:))     

   mmacro_pcond_para%a_cud        = loc(a_cud_trans(:,:))     
   mmacro_pcond_para%a_cu0        = loc(a_cu0_trans(:,:))     

   mmacro_pcond_para%clrw_old     = loc(clrw_old_trans(:,:))     
   mmacro_pcond_para%clri_old     = loc(clri_old_trans(:,:))     
   mmacro_pcond_para%tke          = loc(tke_trans(:,:))            
   mmacro_pcond_para%qtl_flx      = loc(qtl_flx_trans(:,:))            
   mmacro_pcond_para%qti_flx      = loc(qti_flx_trans(:,:))            
   mmacro_pcond_para%cmfr_det     = loc(cmfr_det_trans(:,:))            
   mmacro_pcond_para%qlr_det      = loc(qlr_det_trans(:,:))            
   mmacro_pcond_para%qir_det      = loc(qir_det_trans(:,:))            


   ! Output variables

   mmacro_pcond_para%s_tendout    = loc(s_tendout_trans(:,:))     
   mmacro_pcond_para%qv_tendout   = loc(qv_tendout_trans(:,:))     
   mmacro_pcond_para%ql_tendout   = loc(ql_tendout_trans(:,:))     
   mmacro_pcond_para%qi_tendout   = loc(qi_tendout_trans(:,:))     
   mmacro_pcond_para%nl_tendout   = loc(nl_tendout_trans(:,:))     
   mmacro_pcond_para%ni_tendout   = loc(ni_tendout_trans(:,:))     

   mmacro_pcond_para%qme          = loc(qme_trans(:,:))     
   mmacro_pcond_para%qvadj        = loc(qvadj_trans(:,:))     
   mmacro_pcond_para%qladj        = loc(qladj_trans(:,:))     
   mmacro_pcond_para%qiadj        = loc(qiadj_trans(:,:))     
   mmacro_pcond_para%qllim        = loc(qllim_trans(:,:))     
   mmacro_pcond_para%qilim        = loc(qilim_trans(:,:))     

   mmacro_pcond_para%cld          = loc(cld_trans(:,:))     
   mmacro_pcond_para%al_st_star   = loc(al_st_star_trans(:,:))     
   mmacro_pcond_para%ai_st_star   = loc(ai_st_star_trans(:,:))     
   mmacro_pcond_para%ql_st_star   = loc(ql_st_star_trans(:,:))     
   mmacro_pcond_para%qi_st_star   = loc(qi_st_star_trans(:,:))     

   mmacro_pcond_para%d_rhmin_liq_PBL =  loc(d_rhmin_liq_PBL_trans(:,:)) 
   mmacro_pcond_para%d_rhmin_ice_PBL =  loc(d_rhmin_ice_PBL_trans(:,:))
   mmacro_pcond_para%d_rhmin_liq_det =  loc(d_rhmin_liq_det_trans(:,:))
   mmacro_pcond_para%d_rhmin_ice_det =  loc(d_rhmin_ice_det_trans(:,:))

   mmacro_pcond_para%landfrac     = loc(landfrac(:))
   mmacro_pcond_para%snowh        = loc(snowh(:)) 
   mmacro_pcond_para%wv_para      = loc(wv_para(:)) 

   mmacro_pcond_para%endrunflag      = loc(endrunflag(:)) 

 
   mmacro_pcond_para%dt         = dt

   mmacro_pcond_para%rhmini_const  = rhmini_const
   mmacro_pcond_para%rhmaxi_const  = rhmaxi
   mmacro_pcond_para%rhminl_const  = rhminl_const
   mmacro_pcond_para%rhminl_adj_land_const  = rhminl_adj_land_const
   mmacro_pcond_para%rhminh_const  = rhminh_const

   mmacro_pcond_para%pcols      = pcols
   mmacro_pcond_para%pver       = pver
   mmacro_pcond_para%pverp      = pverp
   mmacro_pcond_para%top_lev    = top_lev
   mmacro_pcond_para%ncol       = ncol
   mmacro_pcond_para%i_rhmini   = i_rhmini
   mmacro_pcond_para%i_rhminl   = i_rhminl

   mmacro_pcond_para%iceopt = iceopt_cpe

   call cnst_get_ind( 'CLDLIQ', ixcldliq )
   call cnst_get_ind( 'CLDICE', ixcldice )
   
   mmacro_pcond_para%qmin1   = qmin(1)
   mmacro_pcond_para%qmin2   = qmin(ixcldliq)
   mmacro_pcond_para%qmin3   = qmin(ixcldice)
   mmacro_pcond_para%premib = premib
   mmacro_pcond_para%premit = premit
   mmacro_pcond_para%icecrit = icecrit_cpe


    if(do_cldice) then
        mmacro_pcond_para%do_cldice_int = 1
    else
        mmacro_pcond_para%do_cldice_int = 0
    endif
    wv_para(1) = epsilo
    wv_para(2) = latvap 
    wv_para(3) = latice
    ...
    wv_para(6) = tmelt
    wv_para(7) = h2otrip
    wv_para(8) = 1.0_r8 - epsilo
    wv_para(9) = 287.04_r8*(7.5_r8*log(10._r8))/cpair
    wv_para(10) = rair 
    wv_para(11) = gravit 
   ...
   call t_stopf("macro_cldwa_trans") 

    call t_startf("macro_cpe")
    !if(masterproc) then
    !write(*,*) "qmin is ", qmin(1), mmacro_pcond_para%qmin1, loc(mmacro_pcond_para%qmin1), loc(mmacro_pcond_para)
    !write(*,*) "qme addr1", loc(qme_trans(:,:))
   call athread_spawn(slave_mmacro_pcond_cpe_c, mmacro_pcond_para)
   call athread_join()
    !write(*,*) "qme addr2", loc(qme_trans(:,:))
   !write(*,*) "qmin is ", qmin(1), mmacro_pcond_para%qmin1, loc(mmacro_pcond_para%qmin1), loc(mmacro_pcond_para)
   !endif
    call t_stopf("macro_cpe")
    !call lwpf_report_summary()
    do i = 1, 64 
        if(endrunflag(i) == 1) then
             write(iulog,*) 'mmacro_pcond in cldwa2m_macro errors' 
             write(iulog,*) 'Impossible case1 in instratus_condensate' 
        else if(endrunflag(i) == 2) then
             write(iulog,*) 'mmacro_pcond in cldwa2m_macro errors' 
             write(iulog,*) 'Impossible case2 in instratus_condensate' 
        else if(endrunflag(i) == 3) then
             write(iulog,*) 'mmacro_pcond in cldwa2m_macro errors' 
             write(iulog,*) 'singular matrix in gaussj 1'
        else if(endrunflag(i) == 4) then
             write(iulog,*) 'mmacro_pcond in cldwa2m_macro errors' 
             write(iulog,*) 'singular matrix in gaussj 2'
        else if(endrunflag(i) == 5) then
             write(iulog,*) 'mmacro_pcond in cldwa2m_macro errors' 
             write(iulog,*) 'Impossible case1 in instratus_condensate' 
        else if(endrunflag(i) == 10) then
             write(iulog,*) 'mmacro_pcond in cldwa2m_macro errors' 
    ...
    do i = 1, 64 
        if(endrunflag(i) .ne. 0) then
            if(endrunflag(i) < 10) then
                call endrun()
            endif
         endif
    enddo
    ...
    s_tendout(i,k)       =  s_tendout_trans(k,i)   
    qv_tendout(i,k)      =  qv_tendout_trans(k,i)  
    ql_tendout(i,k)      =  ql_tendout_trans(k,i)  
    qi_tendout(i,k)      =  qi_tendout_trans(k,i)  
    nl_tendout(i,k)      =  nl_tendout_trans(k,i)  
    ni_tendout(i,k)      =  ni_tendout_trans(k,i)  
                                                                       
    qme  (i,k)           =  qme_trans  (k,i)       
    qvadj(i,k)           =  qvadj_trans(k,i)       
    qladj(i,k)           =  qladj_trans(k,i)       
    qiadj(i,k)           =  qiadj_trans(k,i)       
    qllim(i,k)           =  qllim_trans(k,i)       
    qilim(i,k)           =  qilim_trans(k,i)       
                                                                       
    cld(i,k)             =  cld_trans(k,i)         
    al_st_star(i,k)      =  al_st_star_trans(k,i)  
    ai_st_star(i,k)      =  ai_st_star_trans(k,i)  
    ql_st_star(i,k)      =  ql_st_star_trans(k,i)  
    qi_st_star(i,k)      =  qi_st_star_trans(k,i)  

    d_rhmin_liq_PBL(i,k) = d_rhmin_liq_PBL_trans(k,i) 
    d_rhmin_ice_PBL(i,k) = d_rhmin_ice_PBL_trans(k,i) 
    d_rhmin_liq_det(i,k) = d_rhmin_liq_det_trans(k,i) 
    d_rhmin_ice_det(i,k) = d_rhmin_ice_det_trans(k,i) 
   ...
   !endif
   !write(*,*) "rhmin ", i_rhmini, i_rhminl
   !write(*,*) "spa", mmacro_pcond_para%spa1, mmacro_pcond_para%spa2
   !write(*,*) "top_lev", top_lev
!xukai--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
!#define MPEORG
#ifdef MPEORG
    !call athread_spawn(slave_setftz_f, 0)
    !call athread_join()
    !call set_consist_math_on() 

    call t_startf("macro_mpe")
   cone            = 0.999_r8
   zeros(:ncol,:)  = 0._r8
 ...
 ! Other
   ...
   ! ---------------- !
#ifdef DIFFRES
     call diffres2d(s_tendout_trans, a_t, pcols, pver, &
        ncol, pver, "stend")
      call diffres2d(qv_tendout_trans, a_qv, pcols, pver, &
        ncol, pver, "qvtend")
      call diffres2d(ql_tendout_trans, a_ql, pcols, pver, &
        ncol, pver, "qltend")
      call diffres2d(qi_tendout_trans, a_qi, pcols, pver, &
        ncol, pver, "qitend")
      call diffres2d(nl_tendout_trans, a_nl, pcols, pver, &
        ncol, pver, "hltend")
      call diffres2d(ni_tendout_trans, a_ni, pcols, pver, &
        ncol, pver, "nitend")
#endif
...
!#define DIFFRES
#ifdef DIFFRES
     call diffres2d(s_tendout_trans, rhmini_arr, pcols, pver, &
        ncol, pver, "stend")
      call diffres2d(qv_tendout_trans, rhminl_arr, pcols, pver, &
        ncol, pver, "qvtend")
      call diffres2d(ql_tendout_trans, rhminl_adj_land_arr, pcols, pver, &
        ncol, pver, "qltend")
      call diffres2d(qi_tendout_trans, rhminh_arr, pcols, pver, &
        ncol, pver, "qitend")
      !call diffres2d(nl_tendout_trans, d_rhmin_liq_pbl, pcols, pver, &
      !  ncol, pver, "hltend")
      !call diffres2d(ni_tendout_trans, d_rhmin_ice_pbl, pcols, pver, &
      !  ncol, pver, "nitend")
      !call diffres2d(qme_trans, d_rhmin_liq_det, pcols, pver, &
      !  ncol, pver, "qme")
      !call diffres2d(qvadj_trans, d_rhmin_ice_det, pcols, pver, &
      !  ncol, pver, "qvadj")
#endif
!#undef DIFFRES
...
!#define DIFFRES
#ifdef DIFFRES
      call diffres2d(s_tendout_trans, qv1, pcols, pver, &
        ncol, pver, "stend")
      call diffres2d(qv_tendout_trans, ql1, pcols, pver, &
        ncol, pver, "qvtend")
      call diffres2d(ql_tendout_trans, qi1, pcols, pver, &
        ncol, pver, "qltend")
      call diffres2d(qi_tendout_trans, t1, pcols, pver, &
        ncol, pver, "qitend")
      call diffres2d(nl_tendout_trans, qvten_pwi1, pcols, pver, &
        ncol, pver, "hltend")
      call diffres2d(ni_tendout_trans, qlten_pwi1, pcols, pver, &
        ncol, pver, "nitend")
      call diffres2d(qme_trans, qiten_pwi1, pcols, pver, &
        ncol, pver, "qme")
      call diffres2d(qvadj_trans, tten_pwi1, pcols, pver, &
        ncol, pver, "qvadj")
#endif
!#undef DIFFRES
   ...
   do k = top_lev, pver
      call instratus_condensate( lchnk, ncol, k,                                   &
                                 ...
                                 , tmpp1(:,k), tmpp2(:,k), tmpp3(:,k),tmpp4(:,k))
   ...
   ni_0(:ncol,top_lev:) = max(0._r8,ni1(:ncol,top_lev:)+QQni1(:ncol,top_lev:)*dt)
      ...
      !call diffres2d(s_tendout_trans, tmpp1, pcols, pver, &
      !  ncol, pver, "stend")
      !call diffres3d(qv_tendout_trans, tmpp2, s_tendout_trans, pcols, pver, &
      !  ncol, pver, "stend")
      !call diffres2d(qv_tendout_trans, tmpp2, pcols, pver, &
      !  ncol, pver, "qvtend")
!#define DIFFRES
#ifdef DIFFRES
!      call diffres2d(s_tendout_trans,  tmpp1, pcols, pver, &
...
!      call diffres2d(qv_tendout_trans, tmpp2, pcols, pver, &
!        ncol, pver, "qvtend")
!      call diffres2d(ql_tendout_trans, tmpp3, pcols, pver, &
!        ncol, pver, "qltend")
!      call diffres2d(qi_tendout_trans, tmpp4, pcols, pver, &
!        ncol, pver, "qitend")


      call diffres2d(s_tendout_trans, t_0, pcols, pver, &
        ncol, pver, "stend")
      call diffres2d(qv_tendout_trans, qv_0, pcols, pver, &
        ncol, pver, "qvtend")
      call diffres2d(ql_tendout_trans, ql_0, pcols, pver, &
        ncol, pver, "qltend")
      call diffres2d(qi_tendout_trans, qi_0, pcols, pver, &
        ncol, pver, "qitend")

      call diffres2d(nl_tendout_trans, al_st_0, pcols, pver, &
        ncol, pver, "hltend")
      call diffres2d(ni_tendout_trans, ai_st_0, pcols, pver, &
        ncol, pver, "nitend")
      call diffres2d(qme_trans, ql_st_0, pcols, pver, &
        ncol, pver, "qme")
      call diffres2d(qvadj_trans, qi_st_0, pcols, pver, &
        ncol, pver, "qvadj")
#endif
!#undef DIFFRES
...
!      call diffres2d(s_tendout_trans, qv_0, pcols, pver, &
!        ncol, pver, "stend")
!      call diffres2d(qv_tendout_trans, ql_0, pcols, pver, &
!        ncol, pver, "qvtend")
!      call diffres2d(ql_tendout_trans, qi_0, pcols, pver, &
!        ncol, pver, "qltend")
!      call diffres2d(qi_tendout_trans, t_0, pcols, pver, &
!        ncol, pver, "qitend")

!      call diffres2d(s_tendout_trans, a_qv, pcols, pver, &
!        ncol, pver, "stend")
!      call diffres2d(qv_tendout_trans, a_ql, pcols, pver, &
!        ncol, pver, "qvtend")
!      call diffres2d(ql_tendout_trans, a_qi, pcols, pver, &
!        ncol, pver, "qltend")
!      call diffres2d(qi_tendout_trans, a_t, pcols, pver, &
!        ncol, pver, "qitend")
...
!        ncol, pver, "qitend")
      ...
      call diffres2d(s_tendout_trans, t_05, pcols, pver, &
        ncol, pver, "stend")
      call diffres2d(qv_tendout_trans, qv_05, pcols, pver, &
        ncol, pver, "qvtend")
      call diffres2d(ql_tendout_trans, ql_05, pcols, pver, &
        ncol, pver, "qltend")
      call diffres2d(qi_tendout_trans, qi_05, pcols, pver, &
        ncol, pver, "qitend")

      call diffres2d(nl_tendout_trans, nl_05, pcols, pver, &
        ncol, pver, "hltend")
      call diffres2d(ni_tendout_trans, ni_05, pcols, pver, &
        ncol, pver, "nitend")
#endif
!#undef DIFFRES
        ...
        !tmpp1(:ncol,top_lev:) =ql_0(:ncol,top_lev:)
        !tmpp2(:ncol,top_lev:) = (A_ql(:ncol,top_lev:) + C_ql(:ncol,top_lev:))*dt
        !tmpp3(:ncol,top_lev:) = qi_0(:ncol,top_lev:) 
        !tmpp4(:ncol,top_lev:) = (A_qi(:ncol,top_lev:) +C_qi(:ncol,top_lev:))*dt

!      call diffres2d(nl_tendout_trans, qv_05, pcols, pver, &
!        ncol, pver, "nltend")
!
!      !call diffres2d(ni_tendout_trans, tmpp1, pcols, pver, &
!      !  ncol, pver, "nitend")
!      !call diffres2d(qme_trans, tmpp2, pcols, pver, &
!      !  ncol, pver, "qme")
!
!      !call diffres2dbe(ni_tendout_trans, tmpp1, pcols, pver, &
!      !  1, ncol, top_lev, pver, "nitend")
!      !call diffres2dbe(qme_trans, tmpp2, pcols, pver, &
!      !  1, ncol, top_lev, pver, "qme")
!
!      !call diffres2dbe(ni_tendout_trans, tmpp1, pcols, pver, &
!      !  1, ncol, 1, pver, "nitend")
!      !call diffres2dbe(qme_trans, tmpp2, pcols, pver, &
!      !  1, ncol, 1, pver, "qme")
!
!
!      !call diffres2dbe(ni_tendout_trans, ql_05, pcols, pver, &
!      !  1, ncol, top_lev, pver, "nitend")
!      !call diffres2dbe(qme_trans, qi_05, pcols, pver, &
!      !  1, ncol, top_lev, pver, "qme")
!
!
!      call diffres2d6i(ni_tendout_trans, ql_05, s_tendout_trans, &
!      qv_tendout_trans, tmpp1, tmpp2, pcols, pver, &
!        1, ncol, top_lev, pver, "nitend")
!      call diffres2d6i(qme_trans, qi_05, ql_tendout_trans, qi_tendout_trans, & 
!      tmpp3, tmpp4, pcols, pver, &
!        1, ncol, top_lev, pver, "qme")
!
!
!
!
!
!      call diffres2d(qvadj_trans, t_05, pcols, pver, &
!        ncol, pver, "qvadj")


      !call diffres2d(s_tendout_trans, qv_05, pcols, pver, &
      !  ncol, pver, "stend")
      !call diffres2d(qv_tendout_trans, ql_05, pcols, pver, &
      !  ncol, pver, "qvtend")
      !call diffres2d(ql_tendout_trans, qi_05, pcols, pver, &
      !  ncol, pver, "qltend")
      !call diffres2d(qi_tendout_trans, t_05, pcols, pver, &
      !  ncol, pver, "qitend")
      !call diffres2d(nl_tendout_trans, qmin1, pcols, pver, &
      !  ncol, pver, "nltend")
      !call diffres2d(ni_tendout_trans, qmin2, pcols, pver, &
      !  ncol, pver, "nitend")
      !call diffres2d(qme_trans, qmin3, pcols, pver, &
      !  ncol, pver, "qme")
      !call diffres2d(qvadj_trans, dp, pcols, pver, &
      !  ncol, pver, "qvadj")
           
      !call diffres2dpre(s_tendout_trans, qv_05, pcols, pver, &
      !  ncol, pver, 1.e-19_r8, "stend")
      !call diffres2dpre(qv_tendout_trans, ql_05, pcols, pver, &
      !  ncol, pver, 1.e-19_r8, "qvtend")
      !call diffres2dpre(ql_tendout_trans, qi_05, pcols, pver, &
      !  ncol, pver, 1.e-19_r8,"qltend")
      !call diffres2dpre(qi_tendout_trans, t_05, pcols, pver, &
      !  ncol, pver, 1.e-19_r8,"qitend")
!#define VERRESULT 
#ifdef  VERRESULT 
      do i = 1, ncol
        do k = 1, pver
          t_05(i,k)  = s_tendout_trans(k,i)
          qv_05(i,k) = qv_tendout_trans(k,i)
          ql_05(i,k) = ql_tendout_trans(k,i)
          qi_05(i,k)  = qi_tendout_trans(k,i)
          nl_05(i,k) = nl_tendout_trans(k,i) 
          ni_05(i,k) = ni_tendout_trans(k,i) 
          enddo
        enddo
#endif
!#undef VERRESULT
...
!#define DIFFRES
#ifdef DIFFRES
      call diffres2d(qme_trans, qmin1, pcols, pver, &
        ncol, pver, "qmetr")
      !call diffres2d(qvadj_trans, qmin2, pcols, pver, &
      !  ncol, pver, "qvadj")
      !call diffres2d(qladj_trans, qmin3, pcols, pver, &
      !  ncol, pver, "qladj")
#endif
!#undef  DIFFRES
   ...
   call positive_moisture( ncol, dt, qmin1, qmin2, qmin3, dp, & 
      ...
      !call diffres2dpre(nl_tendout_trans, tmpp1, pcols, pver, &
      !  ncol, pver, 1.e-19_r8, "nltend")

!#define DIFFRES
#ifdef DIFFRES
      call diffres2d(qme_trans, qv_05, pcols, pver, &
        ncol, pver, "qmetr")
      call diffres2d(qvadj_trans, ql_05, pcols, pver, &
        ncol, pver, "qvadj")
      call diffres2d(qladj_trans, qi_05, pcols, pver, &
        ncol, pver, "qladj")
      call diffres2d(qiadj_trans, t_05, pcols, pver, &
        ncol, pver, "qiadj")
      call diffres2d(qllim_trans, a_qv_adj, pcols, pver, &
        ncol, pver, "qllim")
      call diffres2d(qilim_trans, a_ql_adj, pcols, pver, &
        ncol, pver, "qilim")
#endif
!#undef DIFFRES
...
!#define DIFFRES
#ifdef DIFFRES
      call diffres2d(qllim_trans, a_qi_adj, pcols, pver, &
        ncol, pver, "qllim")
      call diffres2d(qilim_trans, a_t_adj, pcols, pver, &
        ncol, pver, "qilim")
#endif
!#undef DIFFRES
...
!#define DIFFRES
#ifdef DIFFRES
      call diffres2d(qvadj_trans, qvwb_aw, pcols, pver, &
        ncol, pver, "qvadj")
      call diffres2d(qiadj_trans, qsat_a, pcols, pver, &
        ncol, pver, "qiadj")
#endif
!#undef DIFFRES
  ...
  do iter = 1, niter
    ...
    !if(iter == 1) then
    !  !call diffres0d(s_tendout_trans(k,i),  aa(1,1), i, k, &
    !  !   "stend")
    !  !call diffres0d(qv_tendout_trans(k,i), aa(1,2), i, k, &
    !  !   "qvtend")
    !  !call diffres0d(ql_tendout_trans(k,i), aa(2,1), i, k, &
    !  !   "qltend")
    !  !call diffres0d(qi_tendout_trans(k,i), aa(2,2), i, k, &
    !  !   "qitend")

    !  call diffres0d(qladj_trans(k,i),  aa(1,1), i, k, &
    !     "stend")
    !  call diffres0d(qiadj_trans(k,i), aa(1,2), i, k, &
    !     "qvtend")
    !  call diffres0d(qllim_trans(k,i), aa(2,1), i, k, &
    !     "qltend")
    !  call diffres0d(qilim_trans(k,i), aa(2,2), i, k, &
    !     "qitend")
    !  call diffres0d(nl_tendout_trans(k,i), bb(1,1), i, k, &
    !     "nltend")
    !  call diffres0d(ni_tendout_trans(k,i), bb(2,1), i, k, &
    !     "nitend")
    ! endif
...
!#define DIFFRES
    ...
    if(iter == 1) then
      ...
      !call diffres2d(qladj_trans,  ai_st, pcols, pver, &
      !   ncol, pver, "stend")
      !call diffres2d(qiadj_trans, al_st, pcols, pver, &
      !   ncol, pver, "qvtend")
      !call diffres2d(qllim_trans, a_st, pcols, pver, &
      !   ncol, pver,"qltend")
      !call diffres2d(qilim_trans, qq, pcols, pver, &
      !   ncol, pver,"qitend")
      !call diffres2d(qme_trans, ql_05, pcols, pver, &
      !   ncol, pver,"qmedj")
      !call diffres2d(qvadj_trans, qqw, pcols, pver, &
      !   ncol, pver,"qvadj")

      call diffres2d(qiadj_trans, a_st_star, pcols, pver, &
         ncol, pver, "qiadj")
      call diffres2d(qllim_trans, ql_st_star, pcols, pver, &
         ncol, pver,"qllim")
      call diffres2d(qilim_trans, qi_st_star, pcols, pver, &
         ncol, pver,"qilim")
      call diffres2d(qvadj_trans, nl_star, pcols, pver, &
         ncol, pver,"qvadj")
      call diffres2d(qme_trans, ni_star, pcols, pver, &
         ncol, pver,"qmedj")
...
!#define DIFFRES
    ...
    if(iter == 1) then
      ...
      !call diffres2d(qladj_trans,  t, pcols, pver, &
      !   ncol, pver, "stend")
      !call diffres2d(qiadj_trans, qv, pcols, pver, &
      !   ncol, pver, "qvtend")
      !call diffres2d(qllim_trans, ql, pcols, pver, &
      !   ncol, pver,"qltend")
      !call diffres2d(qilim_trans, qi, pcols, pver, &
      !   ncol, pver,"qitend")
      !call diffres2d(qme_trans, al_st, pcols, pver, &
      !   ncol, pver,"qmedj")
      !call diffres2d(qvadj_trans, ai_st, pcols, pver, &
      !   ncol, pver,"qvadj")

      !call diffres2d(qiadj_trans, T_dprime, pcols, pver, &
      !   ncol, pver, "qvtend")
      !call diffres2d(qllim_trans, qv_dprime, pcols, pver, &
      !   ncol, pver,"qltend")
      !call diffres2d(qilim_trans, ql_dprime, pcols, pver, &
      !   ncol, pver,"qitend")
      !call diffres2d(qvadj_trans, qi_dprime, pcols, pver, &
      !   ncol, pver,"qvadj")
      !call diffres2d(qme_trans, ni_dprime, pcols, pver, &
      !   ncol, pver,"qmedj")
 
     call diffres2d(qiadj_trans, QQw_prev, pcols, pver, &
         ncol, pver, "qvtend")
     call diffres2d(qllim_trans, QQi_prev, pcols, pver, &
         ncol, pver,"qltend")
     call diffres2d(qilim_trans, QQnl_prev, pcols, pver, &
         ncol, pver,"qitend")
     call diffres2d(qvadj_trans, QQni_prev, pcols, pver, &
         ncol, pver,"qvadj")
 
     !call diffres2d(qiadj_trans, A_nl, pcols, pver, &
     !   ncol, pver, "qvtend")
     !call diffres2d(qllim_trans, C_nl, pcols, pver, &
     !   ncol, pver,"qltend")
     !call diffres2d(qilim_trans, A_ql_adj, pcols, pver, &
     !   ncol, pver,"qitend")
     !call diffres2d(qvadj_trans, A_qi_adj, pcols, pver, &
     !   ncol, pver,"qvadj")
     !call diffres2d(qme_trans, C_qv, pcols, pver, &
     !   ncol, pver,"qmedj")
...
!#define DIFFRES
    ...
    !call diffres2d(qiadj_trans, nl_dprime, pcols, pver, &
    !     ncol, pver, "qvtend")
    !  call diffres2d(qllim_trans,  qqnl2, pcols, pver, &
    !     ncol, pver,"qltend")
    !  call diffres2d(qilim_trans, qi_st_star, pcols, pver, &
    !     ncol, pver,"qitend")
    !  !call diffres2d(qvadj_trans, nl_star, pcols, pver, &
...
!#undef DIFFRES 
   ...
   !tmp1(:ncol,top_lev:) = ( nl_star(:ncol,top_lev:) - nl0(:ncol,top_lev:) )/dt 
   !!tmp1(:nocl,top_lev:) = nl0(:ncol, top_lev:) 
   !!qv0(:) = nl0(:)  
   !tmp2(:ncol, top_lev:) = (A_nl(:ncol, top_lev:)+C_nl(:ncol, top_lev:))


   !!(top_lev:) = ( nl_star(top_lev:) - nl0(top_lev:) )/dt - &
   !!     (A_nl(top_lev:)+C_nl(top_lev:))


   ! call diffres2dpre(T0_trans, tmp1, pcols, pver, &
   !      ncol, pver, 1.d-19, "t0")
   ! call diffres2dpre(qv0_trans, tmp2, pcols, pver, &
   !      ncol, pver,1.d-19, "qv0")
   ! call diffres2dpre(ql0_trans, nl_tendout, pcols, pver, &
   !      ncol, pver,1.d-19, "ql0")
   ! !call diffres2d(qi0_trans, c_nl, pcols, pver, &
   ! !     ncol, pver,"qi0")
   ...
   ni0(:ncol,top_lev:) = ni_star(:ncol,top_lev:)
    ...
    !call diffres2d(s_tendout_trans, s_tendout, pcols, pver, &
    !     ncol, pver,"stend")
    !call diffres2d(qv_tendout_trans, qv_tendout, pcols, pver, &
    !     ncol, pver,"qvtend")
    !call diffres2d(ql_tendout_trans, ql_tendout, pcols, pver, &
    !     ncol, pver,"qltend")
    !call diffres2d(qi_tendout_trans, qi_tendout, pcols, pver, &
    !     ncol, pver,"qitend")
 
    !call diffres2d(nl_tendout_trans, nl_tendout, pcols, pver, &
    !     ncol, pver,"nltend")
    !call diffres2d(ni_tendout_trans, ni_tendout, pcols, pver, &
    !     ncol, pver,"nitend")

    !call diffres2d(qme_trans, qme, pcols, pver, &
    !     ncol, pver,"qme")
    !call diffres2d(qvadj_trans, qvadj, pcols, pver, &
    !     ncol, pver,"qvadj")
    !call diffres2d(qladj_trans, qladj, pcols, pver, &
    !     ncol, pver,"qladj")
    !call diffres2d(qiadj_trans, qiadj, pcols, pver, &
    !     ncol, pver,"qiadj")
    !call diffres2d(qllim_trans, qllim, pcols, pver, &
    !     ncol, pver,"qllim")


    !call diffres2d(cld_trans, cld, pcols, pver, &
    !     ncol, pver,"cld")
    !call diffres2d(al_st_star_trans, al_st_star, pcols, pver, &
    !     ncol, pver,"alst")
    !call diffres2d(ai_st_star_trans, ai_st_star, pcols, pver, &
    !     ncol, pver,"aist")
    !call diffres2d(ql_st_star_trans, ql_st_star, pcols, pver, &
    !     ncol, pver,"qlst")
    !call diffres2d(qi_st_star_trans, qi_st_star, pcols, pver, &
    !     ncol, pver,"qist")


    !call diffres2d(T0_trans, T0, pcols, pver, &
    !     ncol, pver,"t0")
    !call diffres2d(qv0_trans, qv0, pcols, pver, &
    !     ncol, pver,"qv0")
    !call diffres2d(ql0_trans, ql0, pcols, pver, &
    !     ncol, pver,"ql0")
    !call diffres2d(qi0_trans, qi0, pcols, pver, &
    !     ncol, pver,"qi0")
    !call diffres2d(nl0_trans, nl0, pcols, pver, &
    !     ncol, pver,"nl0")
    !call diffres2d(ni0_trans, ni0, pcols, pver, &
    !     ncol, pver,"ni0")
    !call athread_spawn(slave_unsetftz_f, 0)
    !call athread_join()
    !call set_consist_math_off() 
   call t_stopf("macro_mpe")
#endif
...
!=======================================================================================================
   ...
   real(r8), intent(out), optional :: tmp1(pcols)     ! In-stratus IWC [kg/kg]
   real(r8), intent(out), optional :: tmp2(pcols)     ! In-stratus IWC [kg/kg]
   real(r8), intent(out), optional :: tmp3(pcols)     ! In-stratus IWC [kg/kg]
   real(r8), intent(out), optional :: tmp4(pcols)     ! In-stratus IWC [kg/kg]
   ...
   ! if(present(tmp1))  tmp1(:) =  al0_st_nc_in(:)    
   ! if(present(tmp2))  tmp2(:) =  G0_nc_in(:)     
   ! if(present(tmp3))  tmp3(:) =  ai0_st_nc_in(:) 
   ! if(present(tmp3))  tmp4(:) =  0.d0 
