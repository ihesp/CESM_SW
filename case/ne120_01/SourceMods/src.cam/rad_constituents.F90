...
public :: &
   ...
   modes_t, modelist_t, mode_component_t, &
   modes, ma_list
...
subroutine rad_cnst_get_mam_props_by_idx(list_idx, &
   ...
   if (id <= 0 .or. id > numphysprops) then
      write(iulog,*) 'physprop_get: illegal ID value: ', id
   ...
   if (present(lw_hygro_ext))      lw_hygro_ext => physprop(id)%lw_hygro_abs
   ...
   if (present(lw_ext))            lw_ext => physprop(id)%lw_abs
