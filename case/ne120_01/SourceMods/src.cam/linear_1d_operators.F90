...
public :: BoundaryData
public :: BoundaryFlux
public :: lmult_as_diag
public :: add_to_diag
public :: chen_add
public :: chen_new_TriDiagDecomp
public ::  chen_tridiag_finalize
...
interface add_to_diag
  module procedure scalar_add_tridiag
  module procedure diagonal_add_tridiag
end interface

interface lmult_as_diag
  module procedure scalar_lmult_tridiag
  module procedure diagonal_lmult_tridiag
end interface

interface apply
  module procedure apply_tridiag
end interface

interface deriv_diag
  module procedure make_tridiag_deriv_diag
end interface

interface chen_add
  module procedure add_in_place_tridiag_ops
end interface 

interface subtract 
  module procedure subtract_in_place_tridiag_ops
end interface
...
!contains
public :: decomp_left_div
public :: decomp_finalize

contains

! Operator that sets to 0.
subroutine zero_operator(nsys, ncel,op)
  ! Sizes for operator.
  integer, intent(in) :: nsys, ncel

  type(TriDiagOp),intent(inout) :: op

  call chen_new_TriDiagOp(nsys, ncel,op)
...
end subroutine zero_operator

! Operator that does nothing.
subroutine identity_operator(nsys, ncel,op)
  ! Sizes for operator.
  integer, intent(in) :: nsys, ncel
...
end subroutine diagonal_operator
...
subroutine diffusion_operator(coords, d_coef,op,l_bndry, r_bndry)
  ! Grid cell locations.
  ...
  type(BoundaryType), target, intent(in), optional :: l_bndry, r_bndry
  ! Output operator.
  type(TriDiagOp),intent(inout) :: op

  ! Selectors to implement default boundary.
  type(BoundaryType), pointer :: l_bndry_loc, r_bndry_loc
  ...
  call chen_new_TriDiagOp(coords%n, coords%d,op)
  ...
  !call op%deriv_diag()
  call make_tridiag_deriv_diag(op)

end subroutine diffusion_operator
...
subroutine advection_operator(coords, v_coef, op,l_bndry, r_bndry) 
  ! Grid cell locations.
  type(Coords1D), intent(in) :: coords
...
end subroutine advection_operator
...
! off-diagonal terms.
subroutine first_derivative(grid_spacing,op,l_bndry, r_bndry)
  ! Distances between points.
  real(r8), USE_CONTIGUOUS intent(in) :: grid_spacing(:,:)
  ! Boundary conditions.
  type(BoundaryType), intent(in), optional :: l_bndry, r_bndry
  ! Output operator.
  type(TriDiagOp),intent(inout) :: op

  op = first_deriv_op_from_seed(grid_spacing, &
       l_bndry, r_bndry)
...
subroutine second_derivative(grid_spacing,op, l_bndry, r_bndry)
  ! Distances between points.
  real(r8), USE_CONTIGUOUS intent(in) :: grid_spacing(:,:)
  ! Boundary conditions.
  type(BoundaryType), intent(in), optional :: l_bndry, r_bndry
  ...
  op = second_deriv_op_from_seed(grid_spacing,l_bndry, r_bndry)
...
function second_deriv_op_from_seed(grid_spacing,l_bndry, r_bndry) result(op)
  ! Distances between points.
  real(r8), USE_CONTIGUOUS intent(in) :: grid_spacing(:,:)
 ...
 type(TriDiagOp) :: op
  ...
  call chen_new_TriDiagOp(size(grid_spacing, 1), size(grid_spacing, 2) + 1,op)

  ! Left boundary condition.
  call second_make_left(l_bndry_loc,grid_spacing,&
       op%left_bound, op%spr(:,1))

  do k = 2, op%ncel-1
     call second_derivative_seed(grid_spacing(:,k-1), grid_spacing(:,k), &
          op%sub(:,k-1), op%spr(:,k))
  ...
  call second_make_right(r_bndry_loc,grid_spacing,&
...
end function second_deriv_op_from_seed

function first_deriv_op_from_seed(grid_spacing,l_bndry, r_bndry) result(op)
  ! Distances between points.
  real(r8), USE_CONTIGUOUS intent(in) :: grid_spacing(:,:)
  ! Function to locally construct matrix elements.
  ! Boundary conditions.
 ...
 type(TriDiagOp) :: op
  ...
  ! Left boundary condition.
  call first_make_left(l_bndry_loc,grid_spacing,&
       op%left_bound, op%spr(:,1))

  do k = 2, op%ncel-1
     call first_derivative_seed(grid_spacing(:,k-1), grid_spacing(:,k), &
          op%sub(:,k-1), op%spr(:,k))
     !call seed(grid_spacing(:,k-1), grid_spacing(:,k), &
     !     op%sub(:,k-1), op%spr(:,k))
  end do
  ...
  ! Right boundary condition.
  call first_make_right(r_bndry_loc,grid_spacing,&
       op%sub(:,op%ncel-1), op%right_bound)
...
end function first_deriv_op_from_seed
...
subroutine new_BoundaryFixedLayer(width,new_bndry)
  real(r8), USE_CONTIGUOUS intent(in) :: width(:)
  type(BoundaryType) :: new_bndry
!  write(*,*) "new chen 0",__FILE__
  new_bndry%bndry_type = fixed_layer_bndry
!  write(*,*) "new chen 1",__FILE__
  if (.not.allocated(new_bndry%edge_width))allocate(new_bndry%edge_width(size(width)))
  new_bndry%edge_width = width
!  write(*,*) "new chen 2",__FILE__

end subroutine new_BoundaryFixedLayer
...
subroutine first_make_left(self, grid_spacing, term1, term2)
  type(BoundaryType), intent(in) :: self
  real(r8), USE_CONTIGUOUS intent(in) :: grid_spacing(:,:)
  ...
  case (first_order_bndry)
     ...
     call first_derivative_seed(del_minus, del_plus, term1, term2)
...
end subroutine first_make_left

subroutine second_make_left(self, grid_spacing, term1, term2)
  type(BoundaryType), intent(in) :: self
  real(r8), USE_CONTIGUOUS intent(in) :: grid_spacing(:,:)
  real(r8), USE_CONTIGUOUS intent(out) :: term1(:)
  real(r8), USE_CONTIGUOUS intent(out) :: term2(:)
  ...
  case (first_order_bndry)
     ...
     del_plus = grid_spacing(:,1)
     del_minus = del_plus * 4._r8 / epsilon(1._r8)
     call second_derivative_seed(del_minus, del_plus, term1, term2)
  case (extrapolate_bndry)
     ! To extrapolate from the boundary, use distance from the nearest
     ! neighbor (as usual) and the second nearest neighbor (with a negative
     ! sign, since we are using two points on the same side).
...
end subroutine second_make_left


subroutine first_make_right(self, grid_spacing,term1, term2)
  type(BoundaryType), intent(in) :: self
  real(r8), USE_CONTIGUOUS intent(in) :: grid_spacing(:,:)
  real(r8), USE_CONTIGUOUS intent(out) :: term1(:)
  real(r8), USE_CONTIGUOUS intent(out) :: term2(:)
...
end subroutine first_make_right

subroutine second_make_right(self, grid_spacing,term1, term2)
  type(BoundaryType), intent(in) :: self
  real(r8), USE_CONTIGUOUS intent(in) :: grid_spacing(:,:)
  real(r8), USE_CONTIGUOUS intent(out) :: term1(:)
  real(r8), USE_CONTIGUOUS intent(out) :: term2(:)
...
end subroutine second_make_right


subroutine boundary_type_finalize(self)
  type(BoundaryType), intent(inout) :: self
...
subroutine chen_new_TriDiagOp(nsys, ncel,new_TriDiagOp)

  integer, intent(in) :: nsys, ncel
  type(TriDiagOp),intent(out) :: new_TriDiagOp
...
end subroutine chen_new_TriDiagOp

! Deallocator for TriDiagOp.
subroutine chen_tridiag_finalize(self)
  type(TriDiagOp), intent(inout) :: self
...
end subroutine chen_tridiag_finalize
...
subroutine new_BoundaryData(data,new_cond)
  real(r8), USE_CONTIGUOUS intent(in) :: data(:)
  type(BoundaryCond) :: new_cond

  new_cond%cond_type = data_cond
     if(.not.allocated(new_cond%edge_data))  allocate(new_cond%edge_data(size(data)))
  new_cond%edge_data = data

!end function new_BoundaryData
end subroutine new_BoundaryData
...
function new_BoundaryFlux(flux, dt, spacing) result(new_cond)
  ...
  if(.not.allocated(new_cond%edge_data)) allocate(new_cond%edge_data(size(flux)))
...
function apply_left(self, bound_term, array) result(delta_edge)
  type(BoundaryCond), intent(in) :: self
  ...
  case (data_cond)
!    write(*,*) "wwwwwww"
     if(.not.allocated(self%edge_data))  allocate(self%edge_data(size(array, 1)))
...
subroutine boundary_cond_finalize(self)
  type(BoundaryCond), intent(inout) :: self
...
function apply_tridiag(self, array, l_cond, r_cond) result(output)
  ! Operator to apply.
  type(TriDiagOp), intent(in) :: self
  ! Data to act on.
  real(r8), USE_CONTIGUOUS intent(in) :: array(:,:)
  ! Objects representing boundary conditions.
  type(BoundaryCond), target, intent(in), optional :: l_cond, r_cond
  ! Function result.
  real(r8) :: output(size(array, 1), size(array, 2))

  ! Local objects to implement default.
  type(BoundaryCond), pointer :: l_cond_loc, r_cond_loc
  ...
  output(:,1) = self%diag(:,1)*array(:,1) + &
       self%spr(:,1)*array(:,2) + &
       apply_left(l_cond_loc,self%left_bound, array)
  ...
  output(:,self%ncel) = &
       ...
       apply_right(r_cond_loc,self%right_bound, array)
...
function add_tridiag_ops(op1, op2) result(new_op)
  ...
  call add_in_place_tridiag_ops(new_op,op2)
...
subroutine add_in_place_tridiag_ops(self, other)
  ...
  type(TriDiagOp), intent(inout) :: self
  type(TriDiagOp), intent(in) :: other
...
function subtract_tridiag_ops(op1, op2) result(new_op)
  ...
  call subtract_in_place_tridiag_ops(new_op,op2)
...
subroutine chen_new_TriDiagDecomp(op, decomp,graft_decomp)
  type(TriDiagOp), intent(in) :: op
  type(TriDiagDecomp), intent(in), optional :: graft_decomp

  type(TriDiagDecomp),intent(inout) :: decomp
...
end subroutine chen_new_TriDiagDecomp
...
subroutine decomp_left_div(decomp, q, l_cond, r_cond)
  ...
  ! Objects representing boundary conditions.
  type(BoundaryCond), intent(in), optional :: l_cond, r_cond
  ...
  if (present(l_cond)) then
     q(:,1) = q(:,1) + apply_left(l_cond,decomp%ze(:,1), q)
  end if

  if (present(r_cond)) then
     q(:,decomp%ncel) = q(:,decomp%ncel) + &
          apply_right(r_cond,decomp%ca(:,decomp%ncel), q)
  end if
