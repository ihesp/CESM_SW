...
subroutine zm_convi(limcnv_in, zmconv_c0_lnd, zmconv_c0_ocn, zmconv_ke, zmconv_ke_lnd, &
   ...
   if (masterproc) write(iulog,*)'**** ZM: DILUTE Buoyancy Calculation ****'
      ...
      subroutine diffres2d(in1, in2, d1, d2, pcols, pver, sname)
        integer, intent(in) :: pcols, pver
        real(8), intent(in) :: in1(pver, pcols)
        real(8), intent(in) :: in2(pcols, pver)
        integer, intent(in) :: d1, d2
        integer :: i, k
        character(len=*), intent(in) :: sname 
        do i = 1, d1
            do k = 1, d2
                if(abs((in1(k, i) - in2(i,k))/in2(i,k)) > 1.0d-10) then
                    write(*,*) sname, i, k, in1(k,i),  in2(i,k)
                endif
            enddo
        enddo
        end subroutine
     
         subroutine diffres1d(in1, in2, d1, pcols, pver, sname)
        integer, intent(in) :: pcols, pver
        real(8), intent(in) :: in1(pver, pcols)
        real(8), intent(in) :: in2(pcols)
        integer, intent(in) :: d1
        integer :: i, k
        character(len=*), intent(in) :: sname 
        do i = 1, d1
            !do k = 1, d2
                if(abs((in1(1, i) - in2(i))/in2(i)) > 1.0d-10) then
                    write(*,*) sname, i, in1(1,i),  in2(i)
                endif
            !enddo
        enddo
        end subroutine
         subroutine diffresint(in1, in2, d1, pcols, sname)
        integer, intent(in) :: pcols
        integer, intent(in) :: in1(pcols)
        integer, intent(in) :: in2(pcols)
        integer, intent(in) :: d1
        integer :: i, k
        character(len=*), intent(in) :: sname 
        do i = 1, d1
            !do k = 1, d2
                if(abs((in1(i) - in2(i))) > 1.0d-10) then
                    write(*,*) sname, i, in1(i),  in2(i)
                endif
            !enddo
        enddo
        end subroutine

      subroutine diffres2dind(in1, in2, ideep, d1, d2, pcols, pver, sname)
        integer, intent(in) :: pcols, pver
        real(8), intent(in) :: in1(pver, pcols)
        real(8), intent(in) :: in2(pcols, pver)
        ...
        do i = 1, d1
            do k = 1, d2
                if(abs((in1(k, ideep(i)) - in2(i,k))/in2(i,k)) > 1.0d-10) then
                    write(*,*) sname, i, k, in1(k,ideep(i)),  in2(i,k)
                endif
            enddo
        enddo
        end subroutine
...
!xukai+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
      ...
      real(r8) :: t_trans(pver, pcols)    
      real(r8) :: qh_trans(pver, pcols)  
      real(r8) :: pap_trans(pver, pcols)     
      real(r8) :: paph_trans(pver+1, pcols)
      real(r8) :: dpp_trans(pver, pcols)  
      real(r8) :: zm_trans(pver, pcols)
      real(r8) :: zi_trans(pver+1, pcols)

!out arguments


      real(r8) :: qtnd_trans(pver, pcols)  
      real(r8) :: heat_trans(pver, pcols) 
      real(r8) :: mcon_trans(pverp, pcols)
      real(r8) :: dlf_trans(pver, pcols) 
      real(r8) :: pflx_trans(pverp, pcols) 
      real(r8) :: cme_trans(pver, pcols)
      real(r8) :: zdu_trans(pver, pcols)
      real(r8) :: rprd_trans(pver, pcols)  

      real(r8) :: mu_trans(pver, pcols)
      real(r8) :: eu_trans(pver, pcols)
      real(r8) :: du_trans(pver, pcols)
      real(r8) :: md_trans(pver, pcols)
      real(r8) :: ed_trans(pver, pcols)
      real(r8) :: dp_trans(pver, pcols)     
      real(r8) :: ql_trans(pver, pcols)    
!end transpose
      real(r8) :: org_trans(pver, pcols)
      real(r8) :: org2d_trans(pver, pcols)
      real(r8) :: orgt_trans(pver, pcols)

      integer :: lels(pcols) 
      integer :: lcls(pcols) 
      integer :: lons(pcols) 
      integer :: maxis(pcols) 
      integer :: lengath_arr(pcols)
 
      type zm_convr_args
          integer(8) :: t_trans
          integer(8) :: qh_trans 
          integer(8) :: pap_trans     
          integer(8) :: paph_trans
          integer(8) :: dpp_trans
          integer(8) :: zm_trans
          integer(8) :: zi_trans

          integer(8) :: qtnd_trans
          integer(8) :: heat_trans
          integer(8) :: mcon_trans
          integer(8) :: dlf_trans
          integer(8) :: pflx_trans 
          integer(8) :: cme_trans
          integer(8) :: zdu_trans
          integer(8) :: rprd_trans
          integer(8) :: mu_trans
          integer(8) :: eu_trans
          integer(8) :: du_trans
          integer(8) :: md_trans
          ...
          integer(8) :: ql_trans
          integer(8) :: org_trans
          integer(8) :: orgt_trans
          integer(8) :: org2d_trans
          !in
          integer(8) :: geos
          integer(8) :: pblh
          integer(8) :: tpert
      ...
      type(zm_convr_args) :: zm_convr_para 
      integer, external:: slave_zm_convr_parallel

      call t_startf("zm_convr_cpe")

   !call t_startf("zm_convr_cpe")
      zm_convr_para%t_trans    = loc(t_trans(:,:))
      zm_convr_para%qh_trans   = loc(qh_trans(:,:)) 
      zm_convr_para%pap_trans  = loc(pap_trans(:,:)) 
      zm_convr_para%paph_trans = loc(paph_trans(:,:)) 
      zm_convr_para%dpp_trans  = loc(dpp_trans(:,:)) 
      zm_convr_para%zm_trans   = loc(zm_trans(:,:)) 
      zm_convr_para%zi_trans   = loc(zi_trans(:,:))

      zm_convr_para%qtnd_trans = loc(qtnd_trans(:,:)) 
      zm_convr_para%heat_trans = loc(heat_trans(:,:)) 
      zm_convr_para%mcon_trans = loc(mcon_trans(:,:)) 
      zm_convr_para%dlf_trans  = loc(dlf_trans(:,:)) 
      zm_convr_para%pflx_trans = loc(pflx_trans(:,:)) 
      zm_convr_para%cme_trans  = loc(cme_trans(:,:)) 
      zm_convr_para%zdu_trans  = loc(zdu_trans(:,:)) 
      zm_convr_para%rprd_trans = loc(rprd_trans(:,:)) 
      zm_convr_para%mu_trans   = loc(mu_trans(:,:)) 
      zm_convr_para%eu_trans   = loc(eu_trans(:,:)) 
      zm_convr_para%du_trans   = loc(du_trans(:,:)) 
      zm_convr_para%md_trans   = loc(md_trans(:,:)) 
      zm_convr_para%ed_trans   = loc(ed_trans(:,:)) 
      zm_convr_para%dp_trans   = loc(dp_trans(:,:)) 

      zm_convr_para%ql_trans   = loc(ql_trans(:,:)) 
      if(zm_org) then
          zm_convr_para%org_trans = loc(org_trans(:,:))
          zm_convr_para%orgt_trans = loc(orgt_trans(:,:))
          zm_convr_para%org2d_trans = loc(org2d_trans(:,:))
      endif

      zm_convr_para%geos        = loc(geos(:))
      zm_convr_para%pblh        = loc(pblh(:))
      zm_convr_para%tpert       = loc(tpert(:))
      zm_convr_para%landfrac    = loc(landfrac(:))

      zm_convr_para%cape        = loc(cape(:))
      zm_convr_para%dsubcld     = loc(dsubcld(:))
      zm_convr_para%jctop       = loc(jctop(:))
      zm_convr_para%jcbot       = loc(jcbot(:))
      zm_convr_para%prec        = loc(prec(:))
      zm_convr_para%rliq        = loc(rliq(:))


      zm_convr_para%ideep       = loc(ideep(:))
      zm_convr_para%maxg        = loc(maxg(:))
      zm_convr_para%jt          = loc(jt(:))

      zm_convr_para%lengath_arr = loc(lengath_arr(:))

      zm_convr_para%lcl = loc(lcls(:))
      zm_convr_para%lel = loc(lels(:))
      zm_convr_para%lon = loc(lons(:))
      zm_convr_para%mx = loc(maxis(:))

      zm_convr_para%pcols = pcols
      zm_convr_para%pver = pver
      zm_convr_para%pverp = pverp
      zm_convr_para%lchnk = lchnk 
      zm_convr_para%ncol = ncol 

      zm_convr_para%delt = delt 

      zm_convr_para%grav = grav
      zm_convr_para%rgrav = rgrav
      zm_convr_para%cpres = cpres 
      zm_convr_para%rl = rl 
      zm_convr_para%rgas = rgas 
      zm_convr_para%cpliq = cpliq 
      zm_convr_para%cpwv = cpwv
      zm_convr_para%tfreez = tfreez
      zm_convr_para%eps1 = eps1
      zm_convr_para%rh2o = rh2o 
      zm_convr_para%latice = latice 
      zm_convr_para%c0_ocn = c0_ocn
      zm_convr_para%c0_lnd = c0_lnd


!zm_org_mpe is false
      if(zm_org) then
          zm_convr_para%zm_org = 1 
      else
          zm_convr_para%zm_org = 0 
      endif
      if(no_deep_pbl) then
          zm_convr_para%no_deep_pbl = 1 
      else
          zm_convr_para%no_deep_pbl = 0 
      endif


      zm_convr_para%limcnv = limcnv

      do i = 1, pcols
      do k = 1, pver
      t_trans   (k, i) = t(i, k)
      qh_trans  (k, i) = qh(i, k) 
      pap_trans (k, i) = pap(i, k) 
      paph_trans(k, i) = paph(i, k) 
      dpp_trans (k, i) = dpp(i, k) 
      zm_trans  (k, i) = zm(i, k) 
      zi_trans  (k, i) = zi(i, k)
      ql_trans  (k, i) = ql(i, k)
      ...
      if(zm_org) then
          do i = 1, pcols
          do k = 1, pver
          org_trans(k,i) = org(i, k)
          orgt_trans(k,i) = orgt(i, k)
          org2d_trans(k,i) = org2d(i, k)
          enddo
          enddo
      endif
      ...
      call athread_spawn(slave_zm_convr_parallel, zm_convr_para) 
      call athread_join()

      call t_stopf("zm_convr_cpe")
...
!
   ...
   if( cam_physpkg_is('cam3')) then
      ...
      !call t_startf("zm_convr_mpe_dilute")
      ...
      !call t_stopf("zm_convr_mpe_dilute")
!xukai+++++++++++++++++++++++++++++++
            !call diffres2d(qtnd_trans, tp, ncol, pver,&
            !    pcols, pver, "qtnd")
            !call diffres2d(dlf_trans, qstp, ncol, pver, &
            !    pcols, pver, "dlf")

            !call diffres1d(heat_trans, tl, ncol, &
            !    pcols, pver, "heat")
            !call diffres1d(cme_trans, cape, ncol, &
            !    pcols, pver, "cme")
            !call diffresint(lcls, lcl, ncol, &
            !    pcols, "lcl")
            !call diffresint(lels, lel, ncol, &
            !    pcols, "lel")
            !call diffresint(lons, lon, ncol, &
            !    pcols, "lon")
            !call diffresint(maxis, maxi, ncol, &
            !    pcols, "maxi")
              do i = 1, ncol
              do k = 1, pver
                tp(i,k) = qtnd_trans(k,i)
                qstp(i,k) = dlf_trans(k,i)
              enddo
              enddo
              do i = 1, ncol
                tl(i) = heat_trans(1,i)
                cape(i) = cme_trans(1,i)
                lcl(i) = lcls(i)
                lel(i) = lels(i)
                lon(i) = lons(i)
                maxi(i) = maxis(i)
...
!
   ...
   do i=1,ncol
    ...
    call t_stopf("zm_convr_mpe")
   !if (lengath.eq.0) return
   if (lengath > 0) then

      call t_startf("zm_convr_mpe_len")
   do ii=1,lengath
      i=index(ii)
      ideep(ii)=i
   end do
...
!xukai+++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        !    call diffres2dind(rprd_trans, rprd, ideep, lengath, pver,&
        !        pcols, pver, "rprd")
        !    call diffres2dind(pflx_trans, pflx, ideep, lengath, pver,&
        !        pcols, pverp, "pflx")
        !    call diffres2dind(du_trans, du, ideep, lengath, pver,&
        !        pcols, pver, "du")
        !    call diffres2dind(ed_trans, ed, ideep, lengath, pver,&
        !        pcols, pver, "ed")
        !    call diffres2dind(eu_trans, eu, ideep, lengath, pver,&
        !        pcols, pver, "eu")
        !    call diffres2dind(md_trans, md, ideep, lengath, pver,&
        !        pcols, pver, "md")
        !    call diffres2dind(ql_trans, ql, ideep, lengath, pver,&
        !        pcols, pver, "ql")
        !    call diffres2dind(mu_trans, mu, ideep, lengath, pver,&
        !        pcols, pver, "mu")
        !    call diffres2dind(zdu_trans, hsat, ideep, lengath, pver,&
        !        pcols, pver, "zdu")
        !    call diffres2dind(dp_trans, mc, ideep, lengath, pver,&
        !        pcols, pver, "dp")
        !    call diffres2dind(t_trans, qd, ideep, lengath, pver,&
        !        pcols, pver, "t")
        !    call diffres2dind(qh_trans, qs, ideep, lengath, pver,&
        !        pcols, pver, "qh")
        !    call diffres2dind(pap_trans, qu, ideep, lengath, pver,&
        !        pcols, pver, "pap")
        !    call diffres2dind(dpp_trans, sd, ideep, lengath, pver,&
        !        pcols, pver, "dpp")
        !    call diffres2dind(zm_trans, su, ideep, lengath, pver,&
        !        pcols, pver, "zm")
        !do k = msg + 1,pver
        !     do i = 1,lengath
        !        mc(i,k) = dp_trans(k, ideep(i))
        !    enddo
        !enddo
...
! Treat rliq as flux out bottom, to be added back later.
   ...
   rliq(:ncol) = rliq(:ncol) /1000._r8
      ...
      call t_stopf("zm_convr_mpe_len")
    endif
!xukai+++++++++++++++++++++++++++++++
!            call diffres2d(qtnd_trans, qtnd, ncol, pver,&
!                pcols, pver, "qtnd")
!            call diffres2d(dlf_trans, qstp, ncol, pver, &
!                pcols, pver, "dlf")
!
