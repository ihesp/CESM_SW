...
!    type (derivative_t)               :: deriv          ! non staggered derivative struct     (private)
    ...
    real (kind=real_kind) :: lap_tmp(np,np)
...
tol=1.e-12
  ...
  subroutine solver_test(elem,edge1,red,hybrid,deriv,nets,nete)
    ...
    do ie=nets,nete
       do k=1,nlev
          call laplace_sphere_wk(sol(:,:,k,ie),deriv,elem(ie),lap_tmp,var_coef=.false.)
          RHS(:,:,k,ie)=elem(ie)%spheremp(:,:)*sol(:,:,k,ie) + alambda*lap_tmp
          call edgeVpack(edge1, RHS(1,1,1,ie), nlev, 0, ie)
       end do
    end do
    call bndry_exchangeV(cg%hybrid,edge1)
    ...
    do while (congrad(cg,red,maxits,tol))
       do ie=nets,nete
          ...
          do k=1,nlev
             ...
             ! weak laplace operator already includes mass
             ! so only multiply x by mass;
             call laplace_sphere_wk(x,deriv,elem(ie),lap_tmp,var_coef=.false.)
             LHS(:,:,k,ie)=elem(ie)%spheremp(:,:)*x(:,:) + alambda*lap_tmp
    ...
    do ie=nets,nete
       ...
       do k=1,nlev
          call laplace_sphere_wk(sol(:,:,k,ie),deriv,elem(ie),lap_tmp,var_coef=.false.)
          RHS(:,:,k,ie)=sol(:,:,k,ie) + lap_tmp / elem(ie)%spheremp(:,:)
       end do
       !
       !  Initialize CG solver:  set %r = residual from initial guess
       !  if initial guess = 0, then we take %r=RHS
    ...
    do while (congrad(cg,red,maxits,tol))
       ...
       do ie=nets,nete
          ...
          do k=1,nlev
             ...
             ! compute LHS(x) = x + laplace(x)
             call laplace_sphere_wk(x,deriv,elem(ie),lap_tmp,var_coef=.false.)
             LHS(:,:,k,ie)=x(:,:) + lap_tmp /elem(ie)%spheremp(:,:)
